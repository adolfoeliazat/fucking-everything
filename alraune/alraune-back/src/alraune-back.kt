        ...
        File(AlBackPile0.generatedTSFile).writeText(buildString {
            ...
            for (nestedClass in clazz.nestedClasses) {
                ...
                for (prop in nestedClass.memberProperties) {
                    if (prop.name != AlBackToFrontCommand::opcode.name) {
                        val jvmErasure = prop.returnType.jvmErasure
                        val tsType = when (jvmErasure) {
                            String::class -> "string"
                            AlBackToFrontCommandList::class -> "Type[]"
                            else -> wtf("jvmErasure = $jvmErasure    e0a20284-a8ab-44e8-8f17-4a28b1509f4a")
                        }
                        ln("        ${prop.name}: $tsType")
                    }
                }
