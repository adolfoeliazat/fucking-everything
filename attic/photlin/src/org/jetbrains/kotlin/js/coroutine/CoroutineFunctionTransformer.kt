...
class CoroutineFunctionTransformer(private val program: JsProgram, private val function: JsFunction, name: String?) {
    ...
    private fun generateContinuationConstructor(
        ...
        constructor.body.statements.run {
            ...
            this += JsInvocation(Namer.getFunctionCallRef(baseClass), phpThisRef(), interceptorRef).makeStmt()
    ...
    private fun generateCoroutineInstantiation(context: CoroutineTransformationContext) {
        ...
        if (context.metadata.hasReceiver) {
            instantiation.arguments += phpThisRef()
        }
    ...
    private fun generateCoroutineBody(
        ...
        val stateRef = JsNameRef(context.metadata.stateName, phpThisRef())
        ...
        val continueWithException = JsBlock(
                JsAstUtils.assignment(stateRef.deepCopy(), JsNameRef(context.metadata.exceptionStateName, phpThisRef())).makeStmt(),
                JsAstUtils.assignment(JsNameRef(context.metadata.exceptionName, phpThisRef()),
        ...
        val throwResultRef = JsNameRef(context.metadata.exceptionName, phpThisRef())
    ...
    private fun MutableList<JsStatement>.assignToField(fieldName: JsName, value: JsExpression) {
        this += JsAstUtils.assignment(JsNameRef(fieldName, phpThisRef()), value).makeStmt()
    }
