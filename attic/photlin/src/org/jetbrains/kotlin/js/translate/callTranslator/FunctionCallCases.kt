...
//        val receiver = when (dispatchReceiver) {
//            is JsNameRef -> JsNameRef(dispatchReceiver.name!!).setPHPVarRef(true)
//            else -> dispatchReceiver
//        }
//        return PHPMethodCall(receiver, functionName.ident, argumentsInfo.translateArguments)

//        return PHPInvocation(PHPInvocationCallee.VarArrowMethod(dispatchReceiver.ident, functionName.ident), argumentsInfo.translateArguments)

        if (dispatchReceiver is JsNameRef) {
            if (dispatchReceiver.kind == PHPNameRefKind.DUNNO) {
                dispatchReceiver.kind = PHPNameRefKind.VAR
            }
        }
        val pureFqn = pureFqn(functionName, dispatchReceiver)
        return JsInvocation(pureFqn, argumentsInfo.translateArguments)
    }
    ...
    override fun FunctionCallInfo.dispatchReceiver(): JsExpression {
        var dispatchReceiver1 = dispatchReceiver!!
        return buildDefaultCallWithDispatchReceiver(argumentsInfo, dispatchReceiver1, functionName, isNative(), hasSpreadOperator())
    }
