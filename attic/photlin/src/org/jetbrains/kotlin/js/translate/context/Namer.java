...
import static org.jetbrains.kotlin.js.translate.utils.JsAstUtils.pureStaticFqn;
...
public final class Namer {
    ...
    public static JsNameRef getDelegateNameRef(String propertyName) {
        JsNameRef jsNameRef = new JsNameRef(getDelegateName(propertyName), phpThisRef());
        jsNameRef.setKind(PHPNameRefKind.FIELD);
        return jsNameRef;
//        return new JsNameRef(getDelegateName(propertyName), phpThisRef());
    }
    ...
    public static JsExpression throwNPEFunctionRef() {
        return new JsNameRef(THROW_NPE_FUN_NAME, kotlinObject()).setKind(PHPNameRefKind.STATIC);
    }

    @NotNull
    public static JsExpression throwClassCastExceptionFunRef() {
        return new JsNameRef(THROW_CLASS_CAST_EXCEPTION_FUN_NAME, kotlinObject()).setKind(PHPNameRefKind.STATIC);
    }

    @NotNull
    public static JsExpression throwIllegalStateExceptionFunRef() {
        return new JsNameRef(THROW_ILLEGAL_STATE_EXCEPTION_FUN_NAME, kotlinObject()).setKind(PHPNameRefKind.STATIC);
    }

    @NotNull
    public static JsNameRef kotlin(@NotNull JsName name) {
        JsNameRef jsNameRef = pureFqn(name, kotlinObject());
        if (jsNameRef.getQualifier() != null) {
            jsNameRef.setKind(PHPNameRefKind.STATIC);
        }
        return jsNameRef;
    }
    ...
    public static JsNameRef kotlinObject() {
        JsNameRef jsNameRef = pureFqn(KOTLIN_NAME, null);
        return jsNameRef;
    }
    ...
    public JsExpression isInstanceOf(@NotNull JsExpression instance, @NotNull JsExpression type) {
        String typeName;
        if (type instanceof JsNameRef) {
            typeName = ((JsNameRef) type).getIdent();
        } else {
            throw new UnsupportedOperationException("Implement    c9d70fd4-31d8-4920-b3ae-a426e46d959a");
        }
        PHPStaticMethodCall result = new PHPStaticMethodCall("Kotlin", "isType", Arrays.asList(instance, new JsStringLiteral(typeName)));
...
//        return new JsPrefixOperation(JsUnaryOperator.VOID, JsNumberLiteral.ZERO);
    ...
    public static JsNameRef kotlinLong() {
        return new JsNameRef("kotlin__Long");
//        return pureFqn("Long", kotlinObject());
    }

    @NotNull
    public static JsNameRef createInlineFunction() {
        return pureStaticFqn(DEFINE_INLINE_FUNCTION, kotlinObject());
    }
