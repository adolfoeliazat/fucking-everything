...
import photlinc.PHPStaticMethodCall;
...
public final class PatternTranslator extends AbstractTranslator {
    ...
    public JsExpression translateCastExpression(@NotNull KtBinaryExpressionWithTypeRHS expression) {
        ...
        if (isSafeCast(expression)) {
            ...
            onFail = new PHPStaticMethodCall("Kotlin", "throwCCE", Arrays.asList());
//            onFail = new JsInvocation(throwCCEFunRef);
    ...
    @Nullable
    public JsExpression translateFuckCheck(@NotNull JsExpression subject, @NotNull KtTypeReference targetTypeReference) {
        KotlinType targetType = getTypeByReference(bindingContext(), targetTypeReference);

        JsExpression checkFunReference = doFuck(targetType);
    ...
    @Nullable
    private JsExpression doFuck(@NotNull KotlinType type) {
