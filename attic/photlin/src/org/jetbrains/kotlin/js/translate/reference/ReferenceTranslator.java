...
//import photlinc.PHPVarRef;
import photlinc.PHPNameRefKind;
...
public final class ReferenceTranslator {
    ...
    @NotNull
    // @fucking translateAsValueReference
    public static JsExpression translateAsValueReference(@NotNull DeclarationDescriptor descriptor, @NotNull TranslationContext context) {
        ...
        if (alias != null) {
            if (descriptor instanceof VariableDescriptor) {
                if (!(alias instanceof JsNameRef)) throw new IllegalStateException("5f0d3e2b-e3e4-40d7-a308-f7a0232c9987");
                JsNameRef shit = (JsNameRef) alias;
                if (shit.getName() == null) throw new IllegalStateException("16657649-0b96-4fa6-b395-a8baab6f97bd");
                return new JsNameRef(shit.getName()).setKind(PHPNameRefKind.VAR);
            }
            if (alias instanceof JsNameRef) {
                JsNameRef jsNameRef = (JsNameRef) alias;
                jsNameRef.setKind(PHPNameRefKind.VAR);
            }
            return alias;
        }
        ...
        if (descriptor instanceof PropertyDescriptor) {
            ...
            if (context.isFromCurrentModule(property)) {
                JsExpression fuck = context.getInnerReference(property);
