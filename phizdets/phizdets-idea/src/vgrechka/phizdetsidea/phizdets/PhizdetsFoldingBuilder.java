 ...
 * @author yole
 */
public class PhizdetsFoldingBuilder extends CustomFoldingBuilder implements DumbAware {
  ...
  protected boolean isRegionCollapsedByDefault(@NotNull ASTNode node) {
    ...
    if (node.getElementType() == PyElementTypes.STRING_LITERAL_EXPRESSION) {
      ...
      return PhizdetsFoldingSettings.getInstance().isCollapseLongStrings();
    }
    if (node.getElementType() == PyTokenTypes.END_OF_LINE_COMMENT) {
      return PhizdetsFoldingSettings.getInstance().isCollapseSequentialComments();
    }
    ...
    if (FOLDABLE_COLLECTIONS_LITERALS.contains(node.getElementType())) {
      return PhizdetsFoldingSettings.getInstance().isCollapseLongCollections();
    }
    return false;
  }
