 ...
 * @author dcheryasov
 */
public class PhizdetsSdkConfigurator implements DirectoryProjectConfigurator {
  ...
  public void configureProject(final Project project, @NotNull final VirtualFile baseDir, Ref<Module> moduleRef) {
    ...
    final PhizdetsSdkType sdkType = PhizdetsSdkType.getInstance();
    ...
    if (!candidates.isEmpty()) {
      ...
      if (virtualEnvSdk != null) {
        ...
        if (additionalData == null) {
          additionalData = new PhizdetsSdkAdditionalData(PhizdetsSdkFlavor.getFlavor(virtualEnvSdk.getHomePath()));
          ((ProjectJdkImpl)virtualEnvSdk).setSdkAdditionalData(additionalData);
        }
        ((PhizdetsSdkAdditionalData) additionalData).associateWithProject(project);
        return;
      }
      return;
    }
    ...
    final File executableFile = PhizdetsSdkType.findExecutableFile(new File(project.getBasePath(), "bin"), "python");
    if (executableFile != null) {
      final File virtualEnvRoot = PhizdetsSdkType.getVirtualEnvRoot(executableFile.getPath());
