...
import vgrechka.phizdetsidea.phizdets.console.PyCodeExecutor;
import vgrechka.phizdetsidea.phizdets.console.PydevConsoleRunner;
import vgrechka.phizdetsidea.phizdets.console.PhizdetsConsoleRunnerFactory;
import vgrechka.phizdetsidea.phizdets.console.PhizdetsConsoleToolWindow;
...
public class PyExecuteSelectionAction extends AnAction {
  ...
  public void update(AnActionEvent e) {
    ...
    if (editor != null && isPhizdets(editor)) {
  ...
  private static boolean isPhizdets(Editor editor) {
    if (editor == null) {
      return false;
    }
    ...
    ExecutionHelper
      .selectContentDescriptor(dataContext, project, consoles, "Select console to execute in", descriptor -> {
        if (descriptor != null && descriptor.getExecutionConsole() instanceof PyCodeExecutor) {
          ...
          final PhizdetsConsoleToolWindow toolWindow = PhizdetsConsoleToolWindow.getInstance(project);
  ...
  private static Collection<RunContentDescriptor> getConsoles(Project project) {
    PhizdetsConsoleToolWindow toolWindow = PhizdetsConsoleToolWindow.getInstance(project);
  ...
  private static void startConsole(final Project project,
    ...
    if (toolWindow != null && toolWindow.getConsoleContentDescriptors().size() > 0) {
      ...
      PhizdetsConsoleRunnerFactory consoleRunnerFactory = PhizdetsConsoleRunnerFactory.getInstance();
