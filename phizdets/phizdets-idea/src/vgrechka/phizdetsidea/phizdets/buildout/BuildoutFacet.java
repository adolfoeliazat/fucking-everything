 ...
 */
package vgrechka.phizdetsidea.phizdets.buildout;
...
import vgrechka.phizdetsidea.phizdets.buildout.config.BuildoutCfgLanguage;
import vgrechka.phizdetsidea.phizdets.buildout.config.psi.impl.BuildoutCfgFile;
import vgrechka.phizdetsidea.phizdets.facet.FacetLibraryConfigurator;
 ...
 */
public class BuildoutFacet extends LibraryContributingFacet<BuildoutFacetConfiguration> implements PhizdetsPathContributingFacet {

  private static final Logger LOG = Logger.getInstance("#vgrechka.phizdetsidea.phizdets.buildout.BuildoutFacet");
  ...
  @Override
  public List<String> getAdditionalPhizdetsPath() {
  ...
  public void patchCommandLineForBuildout(GeneralCommandLine commandLine) {
    ...
    ParamsGroup scriptParams = params.getParamsGroup(PhizdetsCommandLineState.GROUP_SCRIPT);
    ...
    if (scriptParams.getParameters().size() > 0) {
      ...
      HelperPackage engulfer = PhizdetsHelper.BUILDOUT_ENGULFER;
    ...
    // add pycharm helpers to phizdetspath so that fixGetpass is importable

    PhizdetsEnvUtil.addToPhizdetsPath(env, PhizdetsHelpersLocator.getHelpersRoot().getAbsolutePath());
    /*
    // set prependable paths
    List<String> paths = facet.getAdditionalPhizdetsPath();
    if (paths != null) {
  ...
  public static void attachLibrary(final Module module) {
    ...
    FacetLibraryConfigurator.attachPhizdetsLibrary(module, null, BUILDOUT_LIB_NAME, paths);
  }

  public static void detachLibrary(final Module module) {
    FacetLibraryConfigurator.detachPhizdetsLibrary(module, BUILDOUT_LIB_NAME);
  }
}
