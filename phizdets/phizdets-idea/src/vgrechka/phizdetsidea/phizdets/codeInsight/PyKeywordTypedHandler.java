...
public class PyKeywordTypedHandler extends TypedHandlerDelegate {
  ...
  public Result beforeCharTyped(char character, Project project, Editor editor, PsiFile file, FileType fileType) {
    if (!(fileType instanceof PhizdetsFileType)) return Result.CONTINUE; // else we'd mess up with other file types!
    if (character == ':') {
  ...
  public Result getOverTypeResult(Editor editor, PsiFile file) {
    ...
    // double colons aren't found in Phizdets's syntax, so we can safely overtype a colon everywhere but strings.
