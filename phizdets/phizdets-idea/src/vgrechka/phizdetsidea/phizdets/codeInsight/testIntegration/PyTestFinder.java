 ...
 */
package vgrechka.phizdetsidea.phizdets.codeInsight.testIntegration;
...
import vgrechka.phizdetsidea.phizdets.psi.PyDocStringOwner;
...
import vgrechka.phizdetsidea.phizdets.testing.doctest.PhizdetsDocTestUtil;
import vgrechka.phizdetsidea.phizdets.testing.pytest.PyTestUtil;
      ...
      for (String eachName : names) {
        if (eachName.contains(sourceName)) {
          for (PyClass eachClass : PyClassNameIndex.find(eachName, element.getProject(), GlobalSearchScope.projectScope(element.getProject()))) {
            if (PhizdetsUnitTestUtil.isTestCaseClass(eachClass, null) || PhizdetsDocTestUtil.isDocTestClass(eachClass)) {
      ...
      for (String eachName : names) {
        if (eachName.contains(sourceName)) {
          for (PyFunction eachFunction : PyFunctionNameIndex.find(eachName, element.getProject(), GlobalSearchScope.projectScope(element.getProject()))) {
            if (PhizdetsUnitTestUtil.isTestCaseFunction(
              eachFunction) || PhizdetsDocTestUtil.isDocTestFunction(eachFunction)) {
              classesWithProximities.add(
