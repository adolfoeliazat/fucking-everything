...
public class PyUserSkeletonsUtil {
  ...
  private static final Logger LOG = Logger.getInstance("#vgrechka.phizdetsidea.phizdets.codeInsight.userSkeletons.PyUserSkeletonsUtil");
  ...
  private static List<String> getPossibleUserSkeletonsPaths() {
    ...
    result.add(PhizdetsHelpersLocator.getHelperPath(USER_SKELETONS_DIR));
    return result;
  }
  ...
  public static PyFile getUserSkeletonForModuleQName(@NotNull String qName, @NotNull PsiElement foothold) {
    final Sdk sdk = PhizdetsSdkType.findPhizdetsSdk(foothold);
    if (sdk != null) {
      final Project project = foothold.getProject();
      final PhizdetsSdkPathCache cache = PhizdetsSdkPathCache.getInstance(project, sdk);
