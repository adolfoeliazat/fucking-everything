 ...
 * @author traff
 */
public class PhizdetsPathEditor extends SdkPathEditor {
  private final PathListModel myPathListModel;

  public PhizdetsPathEditor(final String displayName,
  ...
  protected ListCellRenderer createListCellRenderer(JBList list) {
    return new PhizdetsPathListCellRenderer(super.createListCellRenderer(list), myPathListModel);
  }
  ...
  private static class PathListModel {
    ...
    private SdkAdditionalData collectSdkAdditionalData(SdkModificator sdkModificator) {
      PhizdetsSdkAdditionalData data = (PhizdetsSdkAdditionalData)sdkModificator.getSdkAdditionalData();
      if (data == null) {
        data = new PhizdetsSdkAdditionalData(null);
      }
    ...
    public List<VirtualFile> reset(List<VirtualFile> list, SdkModificator modificator) {
      ...
      if (modificator.getSdkAdditionalData() instanceof PhizdetsSdkAdditionalData) {
        PhizdetsSdkAdditionalData data = (PhizdetsSdkAdditionalData)modificator.getSdkAdditionalData();
    ...
    private static boolean isStubPath(@NotNull VirtualFile file) {
      final String path = PhizdetsSdkType.getSkeletonsRootPath(PathManager.getSystemPath());
  ...
  private class PhizdetsPathListCellRenderer extends ListCellRendererWrapper<VirtualFile> {
    private final PathListModel model;

    public PhizdetsPathListCellRenderer(final ListCellRenderer listCellRenderer, PathListModel model) {
      super();
      this.model = model;
    }
