...
import vgrechka.phizdetsidea.phizdets.remote.PyRemoteSourceItem;
import vgrechka.phizdetsidea.phizdets.remote.PhizdetsRemoteInterpreterManager;
import vgrechka.phizdetsidea.phizdets.sdk.*;
...
public class PhizdetsSdkDetailsDialog extends DialogWrapper {
  private static final Logger LOG = Logger.getInstance("#vgrechka.phizdetsidea.phizdets.configuration.PhizdetsSdkDetailsDialog");
  ...
  public PhizdetsSdkDetailsDialog(Project project, NullableConsumer<Sdk> showMoreCallback, Runnable sdkSettingsWereModified) {
    super(project, true);
    myModule = null;
  ...
  public PhizdetsSdkDetailsDialog(Module module, NullableConsumer<Sdk> showMoreCallback, Runnable sdkSettingsWereModified) {
  ...
  private void refreshSdkList() {
    final List<Sdk> phizdetsSdks = myInterpreterList.getAllPhizdetsSdks(myProject);
    Sdk projectSdk = getSdk();
    if (!myShowOtherProjectVirtualenvs) {
      VirtualEnvProjectFilter.removeNotMatching(myProject, phizdetsSdks);
    }
    //noinspection unchecked
    mySdkList.setModel(new CollectionListModel<>(phizdetsSdks));
  ...
  private void addCreatedSdk(@Nullable final Sdk sdk, boolean newVirtualEnv) {
    if (sdk != null) {
      boolean isVirtualEnv = PhizdetsSdkType.isVirtualEnv(sdk);
  ...
  private void editRemoteSdk(Sdk currentSdk) {
    PhizdetsRemoteInterpreterManager remoteInterpreterManager = PhizdetsRemoteInterpreterManager.getInstance();
  ...
  private void setSdkAssociated(SdkModificator modificator, boolean isAssociated) {
    PhizdetsSdkAdditionalData additionalData = (PhizdetsSdkAdditionalData)modificator.getSdkAdditionalData();
    if (additionalData == null) {
      additionalData = new PhizdetsSdkAdditionalData(PhizdetsSdkFlavor.getFlavor(modificator.getHomePath()));
  ...
  private void reloadSdk(@NotNull Sdk currentSdk) {
    /* PhizdetsSdkUpdater.update invalidates the modificator so we need to create a new
      one for further changes
     */
    if (PhizdetsSdkUpdater.update(currentSdk, myModificators.get(currentSdk), myProject, null)){
  ...
  private class ShowPathButton extends AnActionButton implements DumbAware {
    ...
    public void actionPerformed(AnActionEvent e) {
      ...
      final PhizdetsPathEditor pathEditor = createPathEditor(sdk);
      final SdkModificator sdkModificator = myModificators.get(sdk);

      PhizdetsPathDialog dialog = new PhizdetsPathDialog(myProject, pathEditor);
  ...
  private PhizdetsPathEditor createPathEditor(final Sdk sdk) {
    if (PySdkUtil.isRemote(sdk)) {
      return new PyRemotePathEditor(sdk);
    }
    else {
      return new PhizdetsPathEditor("Classes", OrderRootType.CLASSES, FileChooserDescriptorFactory.createAllButJarContentsDescriptor()) {
        @Override
  ...
  private class PyRemotePathEditor extends PhizdetsPathEditor {
    ...
    protected VirtualFile[] doAddItems() {
      ...
      try {
        String[] files = PhizdetsRemoteInterpreterManager
