...
import vgrechka.phizdetsidea.phizdets.packaging.ui.PyInstalledPackagesPanel;
...
public class PyActiveSdkConfigurable implements UnnamedConfigurable {
  ...
  private void layoutPanel() {
      ...
      public void setSelectedItem(Object item) {
        if (SHOW_ALL.equals(item)) {
          ApplicationManager.getApplication().invokeLater(() -> {
            PhizdetsSdkDetailsDialog moreDialog = myModule == null
                                                ? new PhizdetsSdkDetailsDialog(myProject, myAddSdkCallback, getSettingsModifiedCallback())
                                                : new PhizdetsSdkDetailsDialog(myModule, myAddSdkCallback, getSettingsModifiedCallback());
            moreDialog.show();
          });
          return;
        }
    ...
    myDetailsButton.setIcon(PhizdetsIcons.Phizdets.InterpreterGear);
  ...
  private void showDetails() {
    final PhizdetsSdkDetailsDialog moreDialog = myModule == null
                                              ? new PhizdetsSdkDetailsDialog(myProject, myAddSdkCallback, getSettingsModifiedCallback())
                                              : new PhizdetsSdkDetailsDialog(myModule, myAddSdkCallback, getSettingsModifiedCallback());

    PhizdetsSdkDetailsStep.show(myProject, myProjectSdksModel.getSdks(), moreDialog, myMainPanel,
  ...
  public void apply() throws ConfigurationException {
    ...
    if (selectedSdk instanceof PyDetectedSdk) {
      ...
      if (sdkHome != null) {
        selectedSdk = SdkConfigurationUtil.createAndAddSDK(sdkHome.getPath(), PhizdetsSdkType.getInstance());
        if (selectedSdk != null) {
      ...
      PhizdetsSdkUpdater.updateOrShowError(selectedSdk, null, myProject, null);
    }
    if (selectedSdk != null) {
      updateSdkList(false);
  ...
  private void rehighlightVersionSpecific(@Nullable final Sdk newSdk, @Nullable final Sdk prevSdk) {
    if (prevSdk != null && newSdk != null) {
      final PhizdetsSdkFlavor flavor1 = PhizdetsSdkFlavor.getFlavor(newSdk);
      final PhizdetsSdkFlavor flavor2 = PhizdetsSdkFlavor.getFlavor(prevSdk);
  ...
  private void updateSdkList(boolean preserveSelection) {
    final List<Sdk> sdkList = myInterpreterList.getAllPhizdetsSdks(myProject);
    ...
    for (Sdk sdk : sdkList) {
      if (!PhizdetsSdkType.isVirtualEnv(sdk) && !PhizdetsSdkType.isRemote(sdk) && !(sdk instanceof PyDetectedSdk) && separator) {
        items.add(PySdkListCellRenderer.SEPARATOR);
        separator = false;
      }
      if (PhizdetsSdkType.isRemote(sdk) && remoteSeparator) {
        items.add(PySdkListCellRenderer.SEPARATOR);
