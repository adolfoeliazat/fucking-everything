...
import vgrechka.phizdetsidea.phizdets.ReSTService;
...
import vgrechka.phizdetsidea.phizdets.packaging.PyPackageManagerUI;
import vgrechka.phizdetsidea.phizdets.packaging.PyPackageRequirementsSettings;
...
import vgrechka.phizdetsidea.phizdets.testing.TestRunnerService;
...
public class PyIntegratedToolsConfigurable implements SearchableConfigurable {
  ...
  private PhizdetsTestConfigurationsModel myModel;
  ...
  private void initErrorValidation() {
    ...
    facetErrorPanel.getValidatorsManager().registerValidator(new FacetEditorValidator() {
      ...
      public ValidationResult check() {
        final Sdk sdk = PhizdetsSdkType.findPhizdetsSdk(myModule);
        if (sdk != null) {
          final Object selectedItem = myTestRunnerComboBox.getSelectedItem();
          if (PhizdetsTestConfigurationsModel.PY_TEST_NAME.equals(selectedItem)) {
          ...
          else if (PhizdetsTestConfigurationsModel.PYTHONS_NOSETEST_NAME.equals(selectedItem)) {
  ...
  public String getDisplayName() {
    return "Phizdets Integrated Tools";
  }
  ...
  public JComponent createComponent() {
    ...
    myModel = new PhizdetsTestConfigurationsModel(configurations,
    ...
    if (analyzeDoctest.isSelected() != myDocumentationSettings.isAnalyzeDoctest()) {
      ...
      ProjectRootManager.getInstance(myProject).getFileIndex().iterateContent(new ContentIterator() {
        ...
        public boolean processFile(VirtualFile fileOrDir) {
          if (!fileOrDir.isDirectory() && PhizdetsFileType.INSTANCE.getDefaultExtension().equals(fileOrDir.getExtension())) {
            files.add(fileOrDir);
          }
          return true;
        }
