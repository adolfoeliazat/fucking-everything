...
import vgrechka.phizdetsidea.phizdets.console.completion.PhizdetsConsoleAutopopupBlockingHandler;
...
import vgrechka.phizdetsidea.phizdets.debugger.PyDebuggerEditorsProvider;
import vgrechka.phizdetsidea.phizdets.debugger.PyStackFrame;
import vgrechka.phizdetsidea.phizdets.debugger.PyStackFrameInfo;
import vgrechka.phizdetsidea.phizdets.highlighting.PyHighlighter;
 ...
 * @author traff
 */
public class PhizdetsConsoleView extends LanguageConsoleImpl implements ObservableConsoleView, PyCodeExecutor {

  private static final Logger LOG = Logger.getInstance(PhizdetsConsoleView.class);
  ...
  private boolean myIsIPhizdetsOutput;
  ...
  public PhizdetsConsoleView(final Project project, final String title, final Sdk sdk) {
    super(project, title, PhizdetsLanguage.getInstance());

    getVirtualFile().putUserData(LanguageLevel.KEY, PhizdetsSdkType.getLanguageLevelForSdk(sdk));
    // Mark editor as console one, to prevent autopopup completion
    getConsoleEditor().putUserData(PhizdetsConsoleAutopopupBlockingHandler.REPL_KEY, new Object());
    ...
    myPyHighlighter = new PyHighlighter(
      sdk != null && sdk.getVersionString() != null ? LanguageLevel.fromPhizdetsVersion(sdk.getVersionString()) : LanguageLevel.getDefault());
    myScheme = getConsoleEditor().getColorsScheme();
    PhizdetsConsoleData data = PyConsoleUtil.getOrCreateIPhizdetsData(getVirtualFile());
  ...
  public void inputRequested() {
    if (myExecuteActionHandler != null) {
      ...
      if (consoleCommunication instanceof PhizdetsDebugConsoleCommunication) {
  ...
  public void print(@NotNull String text, @NotNull final ConsoleViewContentType outputType) {
    detectIPhizdets(text, outputType);
    if (PyConsoleUtil.detectIPhizdetsEnd(text)) {
      myIsIPhizdetsOutput = false;
      mySourceHighlighter = null;
    }
    else if (PyConsoleUtil.detectIPhizdetsStart(text)) {
      myIsIPhizdetsOutput = true;
    }
    else {
      if (mySourceHighlighter == null || outputType == ConsoleViewContentType.ERROR_OUTPUT) {
        ...
        if (mySourceHighlighter == null && myIsIPhizdetsOutput && PyConsoleUtil.detectSourcePrinting(text)) {
  ...
  public void detectIPhizdets(String text, final ConsoleViewContentType outputType) {
    VirtualFile file = getVirtualFile();
    if (PyConsoleUtil.detectIPhizdetsImported(text, outputType)) {
      PyConsoleUtil.markIPhizdets(file);
    }
    if (PyConsoleUtil.detectIPhizdetsAutomagicOn(text)) {
      PyConsoleUtil.setIPhizdetsAutomagic(file, true);
    }
    if (PyConsoleUtil.detectIPhizdetsAutomagicOff(text)) {
      PyConsoleUtil.setIPhizdetsAutomagic(file, false);
    }
  }

  private boolean detectHyperlink(@NotNull String text) {
    return myIsIPhizdetsOutput && text.startsWith("File:");
  }
