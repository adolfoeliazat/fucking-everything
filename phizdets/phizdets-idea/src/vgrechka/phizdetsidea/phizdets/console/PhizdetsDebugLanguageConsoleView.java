 ...
 * @author traff
 */
public class PhizdetsDebugLanguageConsoleView extends DuplexConsoleView<ConsoleView, PhizdetsConsoleView> implements PyCodeExecutor {

  public static final String DEBUG_CONSOLE_START_COMMAND = "import sys; print('Phizdets %s on %s' % (sys.version, sys.platform))";
  private boolean myDebugConsoleInitialized = false;

  public PhizdetsDebugLanguageConsoleView(final Project project, Sdk sdk, ConsoleView consoleView) {
    super(consoleView, new PhizdetsConsoleView(project, "Phizdets Console", sdk));

    enableConsole(!PyConsoleOptions.getInstance(project).isShowDebugConsoleByDefault());

    getSwitchConsoleActionPresentation().setIcon(PhizdetsIcons.Phizdets.Debug.CommandLine);
  ...
  public PhizdetsDebugLanguageConsoleView(final Project project, Sdk sdk) {
  ...
  @NotNull
  public PhizdetsConsoleView getPydevConsoleView() {
  ...
  public void showStartMessageForFirstExecution(String startCommand, PhizdetsConsoleView console) {
    console.setPrompt("");
  ...
  public void enableConsole(boolean primary) {
    ...
    if (!primary && !isPrimaryConsoleEnabled()) {
      PhizdetsConsoleView console = getPydevConsoleView();
