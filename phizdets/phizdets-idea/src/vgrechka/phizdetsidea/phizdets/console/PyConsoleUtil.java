...
public class PyConsoleUtil {
  ...
  static final Key<PhizdetsConsoleData> PYTHON_CONSOLE_DATA = Key.create("phizdets-console-data");
  ...
  public static boolean detectIPhizdetsImported(@NotNull String text, final ConsoleViewContentType outputType) {
    return text.contains("PyDev console: using IPhizdets ") && outputType == ConsoleViewContentType.ERROR_OUTPUT;
  }
  ...
  public static boolean detectIPhizdetsStart(@NotNull String text) {
    return text.contains("IPhizdets-->");
  }

  public static boolean detectIPhizdetsEnd(@NotNull String text) {
    return text.contains("<--IPhizdets");
  }

  public static boolean detectIPhizdetsAutomagicOn(@NotNull String text) {
    return text.contains("Automagic is ON, % prefix NOT needed for magic functions.");
  }

  public static boolean detectIPhizdetsAutomagicOff(@NotNull String text) {
    return text.contains("Automagic is OFF, % prefix IS needed for magic functions.");
  }

  public static void markIPhizdets(@NotNull VirtualFile file) {
    PhizdetsConsoleData consoleData = getOrCreateIPhizdetsData(file);
    consoleData.setIPhizdetsEnabled(true);
  }

  @NotNull
  public static PhizdetsConsoleData getOrCreateIPhizdetsData(@NotNull VirtualFile file) {
    PhizdetsConsoleData consoleData = file.getUserData(PYTHON_CONSOLE_DATA);
    if (consoleData == null) {
      consoleData = new PhizdetsConsoleData();
      file.putUserData(PYTHON_CONSOLE_DATA, consoleData);
    }
    return consoleData;
  }

  public static void setIPhizdetsAutomagic(@NotNull VirtualFile file, boolean detected) {
    PhizdetsConsoleData consoleData = getOrCreateIPhizdetsData(file);
    consoleData.setIPhizdetsAutomagic(detected);
  }
  ...
  public static AnAction createTabCompletionAction(PhizdetsConsoleView consoleView) {
