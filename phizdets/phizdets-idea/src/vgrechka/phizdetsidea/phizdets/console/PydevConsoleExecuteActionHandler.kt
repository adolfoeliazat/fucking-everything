...
open class PydevConsoleExecuteActionHandler(private val myConsoleView: LanguageConsoleView,
  ...
  private var myIphizdetsInputPromptCount = 1
  ...
  private fun sendLineToConsole(code: ConsoleCommunication.ConsoleCodeFragment) {
    ...
    if (iphizdetsEnabled && !consoleComm.isWaitingForInput && !code.getText().isBlank()) {
      ++myIphizdetsInputPromptCount;
    }
  ...
  fun inputReceived() {
    if (consoleCommunication is PhizdetsDebugConsoleCommunication) {
  ...
  private val iphizdetsEnabled: Boolean
    get() = PyConsoleUtil.getOrCreateIPhizdetsData(myConsoleView.virtualFile).isIPhizdetsEnabled

  private fun iphizdetsInPrompt() {
    ...
    myConsoleView.prompt = "In[$myIphizdetsInputPromptCount]:"
    PyConsoleUtil.scrollDown(myConsoleView.currentEditor)
  }
  ...
  private fun more() {
    val prompt = if (iphizdetsEnabled) {
  ...
  val phizdetsIndent: Int
    get() = CodeStyleSettingsManager.getSettings(project).getIndentSize(PhizdetsFileType.INSTANCE)
