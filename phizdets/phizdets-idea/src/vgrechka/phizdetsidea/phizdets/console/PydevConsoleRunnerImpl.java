...
import vgrechka.phizdetsidea.phizdets.debugger.PySourcePosition;
...
import vgrechka.phizdetsidea.phizdets.run.*;
...
public class PydevConsoleRunnerImpl implements PydevConsoleRunner {
  ...
  public static final String CONSOLE_START_COMMAND = "import sys; print('Phizdets %s on %s' % (sys.version, sys.platform))\n" +
  ...
  private static final String CONSOLE_FEATURE = "phizdets.console";
  ...
  private PhizdetsConsoleView myConsoleView;
  ...
  @Override
  public void open() {
    PhizdetsConsoleToolWindow toolWindow = PhizdetsConsoleToolWindow.getInstance(myProject);
    if (toolWindow != null) {
  ...
  private void showContentDescriptor(RunContentDescriptor contentDescriptor) {
    ToolWindow toolwindow = PhizdetsConsoleToolWindow.getToolWindow(myProject);
    if (toolwindow != null) {
      PhizdetsConsoleToolWindow.getInstance(myProject).init(toolwindow, contentDescriptor);
    }
    else {
      ExecutionManager
  ...
  protected GeneralCommandLine createCommandLine(@NotNull final Sdk sdk,
    ...
    return doCreateConsoleCmdLine(sdk, environmentVariables, workingDir, ports, PhizdetsHelper.CONSOLE);
  }
  ...
  protected GeneralCommandLine doCreateConsoleCmdLine(Sdk sdk,
                                                      ...
                                                      String workingDir,
                                                      int[] ports,
                                                      PhizdetsHelper helper) {
    GeneralCommandLine cmd =
      PhizdetsCommandLineState.createPhizdetsCommandLine(myProject, new PhizdetsConsoleRunParams(myConsoleSettings, workingDir, sdk,
                                                                                           environmentVariables), false,
                                                     PtyCommandLine.isEnabled() && !SystemInfo.isWindows);
    cmd.withWorkDirectory(myWorkingDir);

    ParamsGroup group = cmd.getParametersList().getParamsGroup(PhizdetsCommandLineState.GROUP_SCRIPT);
  ...
  private PhizdetsConsoleView createConsoleView() {
    PhizdetsConsoleView consoleView = new PhizdetsConsoleView(myProject, myTitle, mySdk);
    myPydevConsoleCommunication.setConsoleFile(consoleView.getVirtualFile());
    consoleView.addMessageFilter(new PhizdetsTracebackFilter(myProject));
    return consoleView;
  }
  ...
  private Process createProcess() throws ExecutionException {
    if (PySdkUtil.isRemote(mySdk)) {
      ...
      throw new PhizdetsRemoteInterpreterManager.PyRemoteInterpreterExecutionException();
    }
    else {
      myCommandLine = myGeneralCommandLine.getCommandLineString();
  ...
  private RemoteProcess createRemoteConsoleProcess(PhizdetsRemoteInterpreterManager manager,
                                                   String[] command,
    ...
    commandLine.getParametersList().set(0, PhizdetsRemoteInterpreterManager.toSystemDependent(new File(data.getHelpersPath(),
    ...
    try {
      ...
      // we do not have an option to setup Docker container settings now for Phizdets console so we should bind at least project
      // directory to some path inside the Docker container
      commandLine.putUserData(PhizdetsRemoteInterpreterManager.ADDITIONAL_MAPPINGS, buildDockerPathMappings());
      ...
      if (remoteProcess instanceof Tunnelable) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(String.format("Using tunneled communication for Phizdets console: port %d (=> %d) on IDE side, " +
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(String.format("Using direct communication for Phizdets console: port %d on IDE side, port %d on pydevconsole.py side",
  ...
  private void initAndRun() throws ExecutionException {
    ...
    UIUtil.invokeLaterIfNeeded(() -> {
      ...
      PhizdetsConsoleView consoleView = myConsoleView;
  ...
  private void connect(final String[] statements2execute) {
    if (handshake()) {
      ApplicationManager.getApplication().invokeLater(() -> {
        ...
        final PhizdetsConsoleView consoleView = myConsoleView;
  ...
  private boolean isIndentSubstring(String text) {
    int indentSize = myConsoleExecuteActionHandler.getPhizdetsIndent();
  ...
  private static class NewConsoleAction extends AnAction implements DumbAware {
    public NewConsoleAction() {
      super("New Console", "Creates new phizdets console", AllIcons.General.Add);
    }
    ...
    public void actionPerformed(AnActionEvent e) {
      PydevConsoleRunner runner =
        PhizdetsConsoleRunnerFactory.getInstance().createConsoleRunner(e.getData(CommonDataKeys.PROJECT), e.getData(LangDataKeys.MODULE));
      runner.run();
    }
  }
  ...
  private XDebugSession connectToDebugger() throws ExecutionException {
    ...
    final XDebugSession session = XDebuggerManager.getInstance(myProject).
      startSessionAndShowTab("Phizdets Console Debugger", PhizdetsIcons.Phizdets.Phizdets, null, true, new XDebugProcessStarter() {
        @NotNull
        public XDebugProcess start(@NotNull final XDebugSession session) {
          PhizdetsDebugLanguageConsoleView debugConsoleView = new PhizdetsDebugLanguageConsoleView(myProject, mySdk);
          ...
          PhizdetsDebugConsoleCommunication communication =
  ...
  @Override
  public PhizdetsConsoleView getConsoleView() {
    return myConsoleView;
  }

  public static PhizdetsConsoleRunnerFactory factory() {
    return new PydevConsoleRunnerFactory();
  }

  private static class PhizdetsConsoleRunParams implements PhizdetsRunParams {
    private final PyConsoleOptions.PyConsoleSettings myConsoleSettings;
    private String myWorkingDir;
    private Sdk mySdk;
    private Map<String, String> myEnvironmentVariables;

    public PhizdetsConsoleRunParams(@NotNull PyConsoleOptions.PyConsoleSettings consoleSettings,
