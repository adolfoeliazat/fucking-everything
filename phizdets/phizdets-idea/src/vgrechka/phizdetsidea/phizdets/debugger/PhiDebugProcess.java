...
import vgrechka.phizdetsidea.phizdets.debugger.pydev.*;
...
import vgrechka.phizdetsidea.phizdets.psi.resolve.PyResolveUtil;
...
import vgrechka.phizdetsidea.phizdets.psi.types.PyModuleType;
...
public class PhiDebugProcess extends XDebugProcess implements IPyDebugProcess, ProcessListener {
  ...
  public void showConsole(PyThreadInfo thread) {
    ...
    if (myExecutionConsole instanceof PhizdetsDebugLanguageConsoleView) {
      PhizdetsDebugLanguageConsoleView consoleView = (PhizdetsDebugLanguageConsoleView)myExecutionConsole;
  ...
  public void consoleInputRequested(boolean isStarted) {
    if (myExecutionConsole instanceof PhizdetsDebugLanguageConsoleView) {
      PhizdetsConsoleView consoleView = ((PhizdetsDebugLanguageConsoleView)myExecutionConsole).getPydevConsoleView();
      if (isStarted) {
  ...
  public void runToPosition(@NotNull final XSourcePosition position, @Nullable XSuspendContext context) {
    ...
    if (isConnected() && !mySuspendedThreads.isEmpty()) {
      final PySourcePosition pyPosition = myPositionConverter.convertToPhizdets(position);
  ...
  private String getFunctionName(final XLineBreakpoint breakpoint) {
    ...
    try {
      ...
      final Project project = getSession().getProject();
      if (document != null) {
        if (file.getFileType() == PhizdetsFileType.INSTANCE) {
  ...
  public void addBreakpoint(final PySourcePosition position, XLineBreakpoint breakpoint) {
    breakpoint = myRegisteredBreakpoints.put(position, breakpoint);
    if (isConnected()) {
  ...
  public PyStackFrame createStackFrame(PyStackFrameInfo frameInfo) {
    return new PyStackFrame(getSession().getProject(), this, frameInfo,
                            getPositionConverter().convertFromPhizdets(frameInfo.getPosition()));
  }
