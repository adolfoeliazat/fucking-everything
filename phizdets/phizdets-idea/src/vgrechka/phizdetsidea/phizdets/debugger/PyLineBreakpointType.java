...
public class PyLineBreakpointType extends XLineBreakpointTypeBase {
  ...
  public boolean canPutAt(@NotNull final VirtualFile file, final int line, @NotNull final Project project) {
    ...
    if (document != null) {
      lineHasStoppablePsi(project, file, line, PhizdetsFileType.INSTANCE, document, UNSTOPPABLE_ELEMENTS, UNSTOPPABLE_ELEMENT_TYPES, stoppable
      );
    }
  ...
  public static void lineHasStoppablePsi(@NotNull Project project,
    ...
    if (file.getFileType() == fileType || isPhizdetsScratch(project, file)) {
      XDebuggerUtil.getInstance().iterateLine(project, document, line, psiElement -> {
        ...
        // Phizdets debugger seems to be able to stop on pretty much everything
        stoppable.set(true);
        return false;
      });
  ...
  private static boolean isPhizdetsScratch(@NotNull Project project, @NotNull VirtualFile file) {
    return ScratchUtil.isScratch(file) && LanguageUtil.getLanguageForPsi(project, file) == PhizdetsLanguage.INSTANCE;
  }
