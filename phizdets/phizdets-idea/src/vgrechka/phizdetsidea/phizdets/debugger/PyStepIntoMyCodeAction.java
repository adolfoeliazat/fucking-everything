...
public class PyStepIntoMyCodeAction extends XDebuggerActionBase {
  ...
  public PyStepIntoMyCodeAction() {
    ...
    myStepIntoMyCodeHandler = new XDebuggerSuspendedActionHandler() {
      ...
      public boolean isEnabled(@NotNull Project project, AnActionEvent event) {
        return super.isEnabled(project, event) && isPhizdetsConfigurationSelected(project);
      }
    };
  }
  ...
  protected boolean isPhizdetsConfigurationSelected(Project project) {
    if (project != null) {
      RunnerAndConfigurationSettings settings = RunManager.getInstance(project).getSelectedConfiguration();
      if (settings != null) {
        RunConfiguration runConfiguration = settings.getConfiguration();
        if (runConfiguration instanceof AbstractPhizdetsRunConfiguration) {
          return true;
        }
      }
    }
  ...
  protected boolean isHidden(AnActionEvent event) {
    return !isPhizdetsConfigurationSelected(event.getData(CommonDataKeys.PROJECT));
  }
}
