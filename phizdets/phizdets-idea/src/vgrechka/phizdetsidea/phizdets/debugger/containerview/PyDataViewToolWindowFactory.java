...
public class PyDataViewToolWindowFactory implements ToolWindowFactory {
  ...
  public void createToolWindowContent(@NotNull Project project, @NotNull ToolWindow toolWindow) {
    ...
    addPhizdetsConsoleListener(project);
    ((ToolWindowEx)toolWindow).setAdditionalGearActions(new DefaultActionGroup(new ColoredByDefaultAction()));
  }

  private static void addPhizdetsConsoleListener(@NotNull Project project) {
    final ToolWindow phizdetsConsole = ToolWindowManager.getInstance(project).getToolWindow(PhizdetsConsoleToolWindowFactory.Companion.getID());
    if (phizdetsConsole == null) {
      return;
    }
    phizdetsConsole.getContentManager().addContentManagerListener(new ContentManagerAdapter() {
      @Override
