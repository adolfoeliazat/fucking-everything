...
import vgrechka.phizdetsidea.phizdets.debugger.pydev.AbstractCommand;
import vgrechka.phizdetsidea.phizdets.debugger.pydev.ClientModeMultiProcessDebugger;
import vgrechka.phizdetsidea.phizdets.debugger.pydev.RemoteDebugger;
 ...
 * and all of the Phizdets processes forked and created within it receives incoming connections on the <b>same port</b> using
 ...
 * <p>
 * Each Phizdets process within the debugging script requires <b>single connection</b> from the IDE. When a new Phizdets process is created the
 ...
 * accepted by the Phizdets process but the others are not. Other connections would stay in <i>completed connection queue</i> until a timeout
  ...
  public void waitForConnect() throws IOException {
    if (myState != State.INIT) {
      throw new IllegalStateException(
        "Inappropriate state of Phizdets debugger for connecting to Phizdets debugger: " + myState + "; " + State.INIT + " is expected");
    }
    ...
    LOG.debug(String.format("[%d] Connected to Phizdets debugger script on #%d attempt", hashCode(), i));
  }
