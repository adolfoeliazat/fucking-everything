 ...
 * @author yole
 */
@State(name = "PhizdetsDocumentationMap", storages = @Storage("other.xml"))
public class PhizdetsDocumentationMap implements PersistentStateComponent<PhizdetsDocumentationMap.State> {
  public static PhizdetsDocumentationMap getInstance() {
    return ServiceManager.getService(PhizdetsDocumentationMap.class);
  }
  ...
  public static class State {
    ...
    public State() {
      ...
      addEntry("wx", "http://www.wxphizdets.org/docs/api/{module.name}.{class.name}-class.html#{function.name}");
  ...
  private static String transformPattern(@NotNull String urlPattern, QualifiedName moduleQName, @Nullable PsiNamedElement element,
    ...
    macros.put("phizdets.version", pyVersion);
