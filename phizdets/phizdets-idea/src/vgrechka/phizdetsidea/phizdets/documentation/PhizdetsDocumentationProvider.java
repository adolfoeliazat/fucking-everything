...
import vgrechka.phizdetsidea.phizdets.console.PydevDocumentationProvider;
...
import vgrechka.phizdetsidea.phizdets.toolbox.FP;
 ...
 */
public class PhizdetsDocumentationProvider extends AbstractDocumentationProvider implements ExternalDocumentationProvider {
  ...
  public String getQuickNavigateInfo(PsiElement element, @NotNull PsiElement originalElement) {
    for (PhizdetsDocumentationQuickInfoProvider point : PhizdetsDocumentationQuickInfoProvider.EP_NAME.getExtensions()) {
   ...
   *                Description is the same as {@link PhizdetsDocumentationProvider#getTypeDescription(PyType, TypeEvalContext)} gives but
   ...
   * @param type    type which description will be calculated
   * @param context type evaluation context
   * @return more user-friendly description than result of {@link PhizdetsDocumentationProvider#getTypeName(PyType, TypeEvalContext)}.
  ...
  public static String getUrlFor(PsiElement element, PsiElement originalElement, boolean checkExistence) {
    ...
    final PhizdetsDocumentationMap map = PhizdetsDocumentationMap.getInstance();
    ...
    for (PhizdetsDocumentationLinkProvider provider : Extensions.getExtensions(PhizdetsDocumentationLinkProvider.EP_NAME)) {
    ...
    if (qName != null && qName.getComponentCount() > 0) {
      ApplicationManager.getApplication().invokeLater(() -> {
        final int rc = Messages.showOkCancelDialog(project,
                                                   ...
                                                   "Phizdets External Documentation",
                                                   Messages.getQuestionIcon());
        if (rc == Messages.OK) {
          ShowSettingsUtilImpl.showSettingsDialog(project, PhizdetsDocumentationConfigurable.ID, "");
        }
  ...
  public PsiElement getCustomDocumentationElement(@NotNull Editor editor,
    ...
    if (contextElement != null &&
        PhizdetsDialectsTokenSetProvider.INSTANCE.getKeywordTokens().contains(contextElement.getNode().getElementType())) {
