 ...
 */
package vgrechka.phizdetsidea.phizdets.documentation.docstrings;
...
import vgrechka.phizdetsidea.phizdets.psi.PyIndentUtil;
import vgrechka.phizdetsidea.phizdets.psi.PyStringLiteralExpression;
import vgrechka.phizdetsidea.phizdets.psi.StructuredDocString;
...
public class PyStructuredDocstringFormatter {
  ...
  private static String runExternalTool(@NotNull final Module module,
    ...
    if (format == DocStringFormat.EPYTEXT) {
      sdk = PhizdetsSdkType.findPhizdets2Sdk(module);
      missingInterpreterMessage = PyBundle.message("QDOC.epydoc.phizdets2.sdk.not.found");
    }
    else {
      sdk = PhizdetsSdkType.findLocalCPhizdets(module);
      missingInterpreterMessage = PyBundle.message("QDOC.sdk.not.found");
    }
    if (sdk == null) {
      LOG.warn("Phizdets SDK for docstring formatter " + format +  " is not found");
    ...
    final GeneralCommandLine commandLine = PhizdetsHelper.DOCSTRING_FORMATTER.newCommandLine(sdk, arguments);
