 ...
 */
package vgrechka.phizdetsidea.phizdets.facet;
...
public class FacetLibraryConfigurator {
  ...
  public static void attachPhizdetsLibrary(final Module module,
    ...
    ApplicationManager.getApplication().runWriteAction(new Runnable() {
      public void run() {
        ...
        if (orderEntry != null) {
          ...
          if (lib != null) {
            if (lib instanceof LibraryImpl && ((LibraryImpl)lib).getKind() == null) {   // replace old phizdets libraries with phizdets specific ones
      ...
      private void createNewLibrary(ModifiableRootModel model) {
        ...
        Library lib = projectLibrariesModel.createLibrary(libraryName, PhizdetsLibraryType.getInstance().getKind());
        fillLibrary(module.getProject(), lib, paths);
  ...
  public static void detachPhizdetsLibrary(final Module module, final String libraryName) {
    final ModifiableModelsProvider modelsProvider = ModifiableModelsProvider.SERVICE.getInstance();
