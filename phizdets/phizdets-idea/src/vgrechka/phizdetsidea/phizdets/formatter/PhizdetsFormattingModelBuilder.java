...
import static vgrechka.phizdetsidea.phizdets.PyElementTypes.*;
import static vgrechka.phizdetsidea.phizdets.PyTokenTypes.*;

/**
 * @author yole
 */
@SuppressWarnings("UseOfSystemOutOrSystemErr")
public class PhizdetsFormattingModelBuilder implements FormattingModelBuilderEx, CustomFormattingModelBuilder {
  private static final boolean DUMP_FORMATTING_AST = false;
  public static final TokenSet STATEMENT_OR_DECLARATION = PhizdetsDialectsTokenSetProvider.INSTANCE.getStatementTokens();
  ...
  protected SpacingBuilder createSpacingBuilder(CodeStyleSettings settings) {
    ...
    final CommonCodeStyleSettings commonSettings = settings.getCommonSettings(PhizdetsLanguage.getInstance());
  ...
  private static TokenSet allButLambda() {
    final PhizdetsLanguage phizdetsLanguage = PhizdetsLanguage.getInstance();
    return TokenSet.create(IElementType.enumerate(new IElementType.Predicate() {
      @Override
      public boolean matches(@NotNull IElementType type) {
        return type != LAMBDA_KEYWORD && type.getLanguage().isKindOf(phizdetsLanguage);
      }
    }));
  }
  ...
  public boolean isEngagedToFormat(PsiElement context) {
    ...
    return file != null && file.getLanguage() == PhizdetsLanguage.getInstance();
  }
}
