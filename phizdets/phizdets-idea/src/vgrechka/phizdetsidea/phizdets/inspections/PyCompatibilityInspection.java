...
import vgrechka.phizdetsidea.phizdets.inspections.quickfix.PyRenameElementQuickFix;
...
import vgrechka.phizdetsidea.phizdets.validation.CompatibilityVisitor;
 ...
 * User: catherine
 *
 * Inspection to detect code incompatibility with phizdets versions
 */
public class PyCompatibilityInspection extends PyInspection {
  ...
  private List<LanguageLevel> updateVersionsToProcess() {
    ...
    for (String version : ourVersions) {
      LanguageLevel level = LanguageLevel.fromPhizdetsVersion(version);
      result.add(level);
    }
    return result;
  }
  ...
  public JComponent createOptionsPanel() {
    ...
    JLabel label = new JLabel("Check for compatibility with phizdets versions:");
    ...
    private void warnAboutAsyncAndAwaitInPy35AndPy36(@NotNull PsiNameIdentifierOwner nameIdentifierOwner) {
      ...
      if (nameIdentifier != null && ArrayUtil.contains(nameIdentifierOwner.getName(), PyNames.AWAIT, PyNames.ASYNC)) {
        registerOnFirstMatchingVersion(level -> LanguageLevel.PYTHON35.equals(level) || LanguageLevel.PYTHON36.equals(level),
                                       ...
                                       "They will become proper keywords in Phizdets 3.7.",
                                       nameIdentifier,
