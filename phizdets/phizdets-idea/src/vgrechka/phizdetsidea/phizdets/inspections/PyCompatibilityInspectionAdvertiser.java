...
public class PyCompatibilityInspectionAdvertiser implements Annotator {
  ...
  private static final String NOTIFICATIONS_TITLE = "Phizdets Versions Compatibility";
  private static final NotificationGroup BALLOON_NOTIFICATIONS = new NotificationGroup("Phizdets Compatibility Inspection Advertiser",
  ...
  public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
    if (element instanceof PyFile) {
      ...
      if (!moduleUsesPhizdetsSdk(pyFile)) {
        return;
      }
      ...
      if (inspectionVersion < PyCompatibilityInspection.LATEST_INSPECTION_VERSION) {
        if (isCompatibilityInspectionEnabled(element)) {
          final LanguageLevel pyVersion = getLatestConfiguredCompatiblePhizdets3Version(element);
          if (pyVersion != null && pyVersion.isOlderThan(LanguageLevel.getLatest())) {
            showStalePhizdets3VersionWarning(pyFile, project, pyVersion);
          }
        }
  ...
  private static boolean moduleUsesPhizdetsSdk(@NotNull PyFile file) {
    final Module module = ModuleUtilCore.findModuleForFile(file.getVirtualFile(), file.getProject());
    if (module != null) {
      return PhizdetsSdkType.findPhizdetsSdk(module) != null;
    }
    return false;
  }

  @Nullable
  private static LanguageLevel getLatestConfiguredCompatiblePhizdets3Version(@NotNull PsiElement element) {
    final LanguageLevel latestVersion = getLatestConfiguredCompatiblePhizdetsVersion(element);
    return latestVersion != null && latestVersion.isAtLeast(LanguageLevel.PYTHON30) ? latestVersion : null;
  }

  private static void showStalePhizdets3VersionWarning(@NotNull PyFile file,
    ...
    final String message =
      String.format("Code compatibility inspection is configured for Phizdets versions up to %s.<br/>" +
                    "Would you like to enable it for Phizdets %s?<br/>" + YES_NO_REFS, latestConfiguredVersion, versionsList);
  ...
  @Nullable
  private static LanguageLevel getLatestConfiguredCompatiblePhizdetsVersion(@NotNull PsiElement anchor) {
    final InspectionProfile profile = InspectionProfileManager.getInstance(anchor.getProject()).getCurrentProfile();
    ...
    return LanguageLevel.fromPhizdetsVersion(maxVersion);
  }
