...
import vgrechka.phizdetsidea.phizdets.sdk.flavors.IronPhizdetsSdkFlavor;
...
public class GenerateBinaryStubsFix implements LocalQuickFix {
   ...
   * Be sure to call {@link #isApplicable(vgrechka.phizdetsidea.phizdets.psi.PyImportStatementBase)} first to make sure this statement is supported
   *
  ...
  /**
   * @param importStatementBase statement to fix
   * @param qualifiedName       name should be fixed (one of {@link vgrechka.phizdetsidea.phizdets.psi.PyImportStatementBase#getFullyQualifiedObjectNames()})
   */
  private GenerateBinaryStubsFix(@NotNull final PyImportStatementBase importStatementBase, @NotNull final String qualifiedName) {
    ...
    mySdk = getPhizdetsSdk(importStatementBase);
  }
  ...
  private boolean generateSkeletonsForList(@NotNull final PySkeletonRefresher refresher,
    ...
    GeneralCommandLine cmd = PhizdetsHelper.EXTRA_SYSPATH.newCommandLine(homePath, Lists.newArrayList(myQualifiedName));
    final ProcessOutput runResult = PySdkUtil.getProcessOutput(cmd,
                                                               new File(homePath).getParent(),
                                                               PhizdetsSdkType.getVirtualEnvExtraEnv(homePath), 5000
    );
  ...
  private List<String> collectAssemblyReferences(PsiFile file) {
    if (!(PhizdetsSdkFlavor.getFlavor(mySdk) instanceof IronPhizdetsSdkFlavor)) {
  ...
  public static boolean isApplicable(@NotNull final PyImportStatementBase importStatementBase) {
    ...
    final Sdk sdk = getPhizdetsSdk(importStatementBase);
    if (sdk == null) {
      return false;
    }
    final PhizdetsSdkFlavor flavor = PhizdetsSdkFlavor.getFlavor(sdk);
    if (flavor instanceof IronPhizdetsSdkFlavor) {
      return true;
    }
  ...
  @Nullable
  private static Sdk getPhizdetsSdk(@NotNull final PsiElement element) {
    final Module module = ModuleUtilCore.findModuleForPsiElement(element);
    return (module == null) ? null : PhizdetsSdkType.findPhizdetsSdk(module);
  }
}
