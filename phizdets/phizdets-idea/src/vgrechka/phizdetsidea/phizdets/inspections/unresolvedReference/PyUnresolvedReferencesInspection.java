...
import vgrechka.phizdetsidea.phizdets.PyCustomType;
import vgrechka.phizdetsidea.phizdets.PyNames;
import vgrechka.phizdetsidea.phizdets.codeInsight.PyCodeInsightSettings;
...
import vgrechka.phizdetsidea.phizdets.codeInsight.imports.AutoImportHintAction;
import vgrechka.phizdetsidea.phizdets.codeInsight.imports.AutoImportQuickFix;
import vgrechka.phizdetsidea.phizdets.codeInsight.imports.OptimizeImportsQuickFix;
import vgrechka.phizdetsidea.phizdets.codeInsight.imports.PhizdetsImportUtils;
import vgrechka.phizdetsidea.phizdets.console.PydevConsoleRunner;
import vgrechka.phizdetsidea.phizdets.documentation.docstrings.DocStringParameterReference;
import vgrechka.phizdetsidea.phizdets.documentation.docstrings.DocStringTypeReference;
...
import vgrechka.phizdetsidea.phizdets.packaging.PyRequirement;
import vgrechka.phizdetsidea.phizdets.psi.*;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyBuiltinCache;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyImportStatementNavigator;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyImportedModule;
...
import vgrechka.phizdetsidea.phizdets.psi.resolve.ImportedResolveResult;
...
import static vgrechka.phizdetsidea.phizdets.inspections.quickfix.AddIgnoredIdentifierQuickFix.END_WILDCARD;
...
public class PyUnresolvedReferencesInspection extends PyInspection {
  ...
  public static class Visitor extends PyInspectionVisitor {
    ...
    public boolean isEnabled(@NotNull PsiElement anchor) {
      if (myIsEnabled == null) {
        ...
        if (PySkeletonRefresher.isGeneratingSkeletons()) {
          ...
          myIsEnabled = PhizdetsSdkType.findPhizdetsSdk(anchor) != null || PyUtil.isInScratchFile(anchor);
        }
        else {
          myIsEnabled = true;
        }
    ...
    private void registerUnresolvedReferenceProblem(@NotNull PyElement node, @NotNull final PsiReference reference,
      ...
      if (description == null) {
        ...
        if (element instanceof PyQualifiedExpression) {
          // TODO: Add __qualname__ for Phizdets 3.3 to the skeleton of <class 'object'>, introduce a pseudo-class skeleton for
          // <class 'function'>
        ...
        if (!markedQualified) {
          ...
          if (PhizdetsImportUtils.isImportable(element)) {
    ...
    private static void addAutoImportFix(PyElement node, PsiReference reference, List<LocalQuickFix> actions) {
      ...
      AutoImportQuickFix importFix = PhizdetsImportUtils.proposeImportFix(node, reference);
      if (importFix != null) {
        ...
        if (!components.isEmpty()) {
          final String packageName = components.get(0);
          final Module module = ModuleUtilCore.findModuleForPsiElement(node);
          if (PyPIPackageUtil.INSTANCE.isInPyPI(packageName) && PhizdetsSdkType.findPhizdetsSdk(module) != null) {
            ...
            if (packageAlias != null && PyPIPackageUtil.INSTANCE.isInPyPI(packageName) && PhizdetsSdkType.findPhizdetsSdk(module) != null) {
    ...
    private List<PsiElement> collectUnusedImportElements() {
      ...
      // PY-1315 Unused imports inspection shouldn't work in phizdets REPL console
