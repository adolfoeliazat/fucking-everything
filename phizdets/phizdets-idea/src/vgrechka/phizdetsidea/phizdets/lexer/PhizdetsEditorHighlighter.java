 ...
 * User : catherine
 */
public class PhizdetsEditorHighlighter extends LexerEditorHighlighter {

  public PhizdetsEditorHighlighter(@NotNull EditorColorsScheme scheme, @Nullable Project project, @Nullable VirtualFile file) {
    super(SyntaxHighlighterFactory.getSyntaxHighlighter(file != null ? file.getFileType() : PhizdetsFileType.INSTANCE,
                                                               project,
                                                               file),
          scheme);
  }
  ...
  public static final Key<Boolean> KEY = new Key<>("phizdets.future.import");
  @Override
  public void documentChanged(DocumentEvent e) {
    synchronized (this) {
      ...
      if (l instanceof LayeredLexer) {
        ...
        if (delegate instanceof PhizdetsHighlightingLexer &&
            (((PhizdetsHighlightingLexer)delegate).getImportOffset() > e.getOffset()
             || hasUnicodeImport != hadUnicodeImport)) {
          ((PhizdetsHighlightingLexer)delegate).clearState(e.getDocument().getTextLength());
