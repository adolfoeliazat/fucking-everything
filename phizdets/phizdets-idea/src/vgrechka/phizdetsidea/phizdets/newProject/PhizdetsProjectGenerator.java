 ...
 */
package vgrechka.phizdetsidea.phizdets.newProject;
...
import vgrechka.phizdetsidea.phizdets.remote.*;
...
/**
 * This class encapsulates remote settings, so one should extend it for any phizdets project that supports remote generation, at least
 ...
 * <br/>
 * If your project does not support remote projects generation, be sure to set flag in ctor:{@link #PhizdetsProjectGenerator(boolean)}
 * <br/>
 ...
 */
public abstract class PhizdetsProjectGenerator<T extends PyNewProjectSettings> implements DirectoryProjectGenerator<T> {
  ...
  protected PhizdetsProjectGenerator() {
    this(false);
  }
   ...
   */
  protected PhizdetsProjectGenerator(final boolean allowRemoteProjectCreation) {
  ...
  public void checkProjectCanBeCreatedOnSdk(@NotNull final Sdk sdk,
    ...
    final PyProjectSynchronizer synchronizer = PhizdetsRemoteInterpreterManager.getSynchronizerInstance(sdk);
  ...
  public final void generateProject(@NotNull final Project project,
    ...
    final PhizdetsRemoteInterpreterManager remoteManager = PhizdetsRemoteInterpreterManager.getInstance();
  ...
  protected void configureProjectNoSettings(@NotNull final Project project,
    ...
    throw new IllegalStateException(String.format("%s does not support project creation with out of settings. " +
                                                  "See %s doc for detail", getClass(), PhizdetsProjectGenerator.class));
  }
   ...
   *                     Otherwise, be sure to use it move code between local (java) and remote (phizdets) side.
   *                     Remote interpreters can't be used with out of it. Contract is following:
   *                     <ol>
   *                     <li>Create some code on phizdets (remote) side using helpers</li>
  ...
  @Nullable
  public BooleanFunction<PhizdetsProjectGenerator> beforeProjectGenerated(@Nullable final Sdk sdk) {
    return null;
  }
