...
import vgrechka.phizdetsidea.phizdets.sdk.PySdkListCellRenderer;
import vgrechka.phizdetsidea.phizdets.sdk.PhizdetsSdkDetailsStep;
 ...
 * @author yole
 */
public class PhizdetsSdkChooserCombo extends ComboboxWithBrowseButton {
  private final List<ActionListener> myChangedListeners = ContainerUtil.createLockFreeCopyOnWriteList();
  private static final Logger LOG = Logger.getInstance(PhizdetsSdkChooserCombo.class);

  @SuppressWarnings("unchecked")
  public PhizdetsSdkChooserCombo(final Project project, List<Sdk> sdks, final Condition<Sdk> acceptableSdkCondition) {
    super(new ComboBox<>());
  ...
  private void showOptions(final Project project) {
    ...
    PhizdetsSdkDetailsStep.show(project, sdks, null, this, getButton().getLocationOnScreen(), sdk -> {
      if (sdk == null) return;
      ...
      if (projectSdksModel.findSdk(sdk) == null) {
        ...
        try {
          ...
          LOG.error("Error adding new phizdets interpreter " + e.getMessage());
        }
      }
      final List<Sdk> committedSdks = interpreterList.getAllPhizdetsSdks();
