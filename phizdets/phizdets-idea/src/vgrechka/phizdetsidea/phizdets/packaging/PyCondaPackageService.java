...
import vgrechka.phizdetsidea.phizdets.packaging.ui.PyCondaManagementService;
...
public class PyCondaPackageService implements PersistentStateComponent<PyCondaPackageService> {
  ...
  @Nullable
  public static String getCondaPhizdets() {
    final String conda = getSystemCondaExecutable();
    final String phizdetsName = SystemInfo.isWindows ? "python.exe" : "phizdets";
    if (conda != null) {
      ...
      if (condaFile != null) {
        ...
        final VirtualFile phizdets = condaDir.findChild(phizdetsName);
        if (phizdets != null) {
          return phizdets.getPath();
        }
      }
    }
    return getCondaExecutable(phizdetsName);
  }
  ...
  private static String findExecutable(String condaName, @Nullable final VirtualFile condaFolder) {
    if (condaFolder != null) {
      ...
      if (bin != null) {
        ...
        final String executableFile = PhizdetsSdkType.getExecutablePath(directoryPath, condaName);
  ...
  public void updatePackagesCache() {
    final String condaPhizdets = getCondaPhizdets();
    if (condaPhizdets == null) {
      LOG.warn("Could not find conda phizdets interpreter");
      return;
    }
    final String path = PhizdetsHelpersLocator.getHelperPath("conda_packaging_tool.py");
    final String runDirectory = new File(condaPhizdets).getParent();
    final String[] command = {condaPhizdets, path, "listall"};
  ...
  public void updateChannels() {
    final String condaPhizdets = getCondaPhizdets();
    if (condaPhizdets == null) return;
    final String path = PhizdetsHelpersLocator.getHelperPath("conda_packaging_tool.py");
    final String runDirectory = new File(condaPhizdets).getParent();
    final ProcessOutput output = PySdkUtil.getProcessOutput(runDirectory, new String[]{condaPhizdets, path, "channels"});
