 ...
 */
package vgrechka.phizdetsidea.phizdets.packaging;
...
public class PyPackageManagerImpl extends PyPackageManager {
  // Phizdets 2.4-2.5 compatible versions
  ...
  public void refresh() {
    ...
    application.invokeLater(() -> {
      ...
      PhizdetsSdkType.getInstance().setupSdkPaths(sdk);
    });
  }
  ...
  public void installManagement() throws ExecutionException {
    final Sdk sdk = getSdk();
    final boolean pre26 = PhizdetsSdkType.getLanguageLevelForSdk(sdk).isOlderThan(LanguageLevel.PYTHON26);
  ...
  protected void installManagement(@NotNull String name) throws ExecutionException {
    ...
    try {
      ...
      getPhizdetsProcessResult(fileName, Collections.singletonList(INSTALL), true, true, dirName + name);
    }
    finally {
  ...
  public String createVirtualEnv(@NotNull String destinationDir, boolean useGlobalSite) throws ExecutionException {
    final List<String> args = new ArrayList<>();
    final Sdk sdk = getSdk();
    final LanguageLevel languageLevel = PhizdetsSdkType.getLanguageLevelForSdk(sdk);
    ...
    if (usePyVenv) {
      ...
      try {
        ...
        getPhizdetsProcessResult(fileName, args, false, true, dirName + name);
      }
      finally {
        FileUtil.delete(new File(dirName));
      }
    ...
    final String binary = PhizdetsSdkType.getPhizdetsExecutable(destinationDir);
    ...
    if (usePyVenv) {
      // Still no 'packaging' and 'pysetup3' for Phizdets 3.3rc1, see PEP 405
      final VirtualFile binaryFile = LocalFileSystem.getInstance().refreshAndFindFileByPath(path);
      if (binaryFile != null) {
        final ProjectJdkImpl tmpSdk = new ProjectJdkImpl("", PhizdetsSdkType.getInstance());
  ...
  private String getHelperResult(@NotNull String helper, @NotNull List<String> args, boolean askForSudo,
    ...
    return getPhizdetsProcessResult(helperPath, args, askForSudo, showProgress, parentDir);
  }

  @Nullable
  protected String getHelperPath(String helper) throws ExecutionException {
    return PhizdetsHelpersLocator.getHelperPath(helper);
  }

  @NotNull
  private String getPhizdetsProcessResult(@NotNull String path, @NotNull List<String> args, boolean askForSudo,
                                        boolean showProgress, @Nullable String workingDir) throws ExecutionException {
    final ProcessOutput output = getPhizdetsProcessOutput(path, args, askForSudo, showProgress, workingDir);
  ...
  @NotNull
  protected ProcessOutput getPhizdetsProcessOutput(@NotNull String helperPath, @NotNull List<String> args, boolean askForSudo,
                                                 boolean showProgress, @Nullable String workingDir) throws ExecutionException {
    final String homePath = getSdk().getHomePath();
    if (homePath == null) {
      throw new ExecutionException("Cannot find Phizdets interpreter for SDK " + mySdk.getName());
    }
    if (workingDir == null) {
    ...
    try {
      ...
      PhizdetsEnvUtil.setPhizdetsUnbuffered(environment);
      PhizdetsEnvUtil.setPhizdetsDontWriteBytecode(environment);
      PhizdetsEnvUtil.resetHomePathChanges(homePath, environment);
