...
import vgrechka.phizdetsidea.phizdets.run.PyRemoteProcessStarterManagerUtil;
...
public class PyRemotePackageManagerImpl extends PyPackageManagerImpl {
  ...
  @NotNull
  @Override
  protected ProcessOutput getPhizdetsProcessOutput(@NotNull String helperPath,
    ...
    if (homePath == null) {
      throw new ExecutionException("Cannot find Phizdets interpreter for SDK " + sdk.getName());
    }
    final SdkAdditionalData sdkData = sdk.getSdkAdditionalData();
    if (sdkData instanceof PyRemoteSdkAdditionalDataBase) { //remote interpreter
      ...
      if (CaseCollector.useRemoteCredentials((PyRemoteSdkAdditionalDataBase)sdkData)) {
        ...
        if (manager != null && remoteSdkCredentials != null) {
          ...
          throw new PyExecutionException(PhizdetsRemoteInterpreterManager.WEB_DEPLOYMENT_PLUGIN_IS_DISABLED, helperPath, args);
        }
      }
