...
public class PyInstalledPackagesPanel extends InstalledPackagesPanel {
    ...
    public void run(@NotNull final Sdk sdk) {
      final PyPackageManagerUI ui = new PyPackageManagerUI(myProject, sdk, new PyPackageManagerUI.Listener() {
        ...
        public void finished(List<ExecutionException> exceptions) {
          ...
          if (description != null) {
            PackagesNotificationPanel.showError("Failed to install Phizdets packaging tools", description);
          }
  ...
  public void updateNotifications(@Nullable final Sdk selectedSdk) {
    ...
    application.executeOnPooledThread(() -> {
      ...
      try {
        ...
        if (!myHasManagement) {
          throw new PyExecutionException("Phizdets packaging tools not found", "pip", Collections.<String>emptyList(), "", "", 0,
      ...
      application.invokeLater(() -> {
        if (selectedSdk == getSelectedSdk()) {
          myNotificationArea.hide();
          if (problem != null) {
            final boolean invalid = PhizdetsSdkType.isInvalid(selectedSdk);
            if (!invalid) {
  ...
  protected boolean canUninstallPackage(InstalledPackage pkg) {
    ...
    if (PhizdetsSdkType.isVirtualEnv(getSelectedSdk()) && pkg instanceof PyPackage) {
