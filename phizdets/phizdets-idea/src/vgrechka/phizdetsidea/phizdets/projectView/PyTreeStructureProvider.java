 ...
 */
package vgrechka.phizdetsidea.phizdets.projectView;
...
public class PyTreeStructureProvider implements SelectableTreeStructureProvider, DumbAware {
  ...
  public Collection<AbstractTreeNode> modify(@NotNull AbstractTreeNode parent,
    ...
    final Sdk sdk = getPhizdetsSdk(parent);
  ...
  @Nullable
  private static Sdk getPhizdetsSdk(@NotNull AbstractTreeNode node) {
    if (node instanceof NamedLibraryElementNode) {
      ...
      if (value != null) {
        ...
        if (entry instanceof JdkOrderEntry) {
          ...
          if (type instanceof PhizdetsSdkType) {
            return sdk;
          }
        }
      }
  ...
  private static Collection<AbstractTreeNode> hideSkeletons(@NotNull Collection<AbstractTreeNode> children) {
    ...
    for (AbstractTreeNode child : children) {
      if (child instanceof PsiDirectoryNode) {
        ...
        if (dirParent != null && dirParent.getName().equals(PhizdetsSdkType.SKELETON_DIR_NAME)) {
          continue;
        }

        if (dirParent != null && dirParent.getName().equals(PhizdetsSdkType.REMOTE_SOURCES_DIR_NAME)) {
          continue;
        }
        if (dirParent != null) {
          VirtualFile grandParent = dirParent.getParent();

          if (grandParent != null && grandParent.getName().equals(PhizdetsSdkType.REMOTE_SOURCES_DIR_NAME)) {
            continue;
          }
        }
      }
