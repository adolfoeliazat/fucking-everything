...
import vgrechka.phizdetsidea.phizdets.console.parsing.PyConsoleParser;
import vgrechka.phizdetsidea.phizdets.console.parsing.PhizdetsConsoleData;
import vgrechka.phizdetsidea.phizdets.console.parsing.PhizdetsConsoleLexer;
import vgrechka.phizdetsidea.phizdets.parsing.PyParser;
import vgrechka.phizdetsidea.phizdets.parsing.StatementParsing;
import vgrechka.phizdetsidea.phizdets.psi.impl.stubs.PyFileStubBuilder;
import vgrechka.phizdetsidea.phizdets.psi.impl.stubs.PyFileStubImpl;
import vgrechka.phizdetsidea.phizdets.psi.stubs.PyFileStub;
...
public class PyFileElementType extends IStubFileElementType<PyFileStub> {
  public static PyFileElementType INSTANCE = new PyFileElementType(PhizdetsLanguage.getInstance());
  ...
  public ASTNode parseContents(ASTNode node) {
    ...
    if (PydevConsoleRunner.isPhizdetsConsole(node)) {
      return parseConsoleCode(node, PydevConsoleRunner.getPhizdetsConsoleData(node));
    }
    else {
      ...
      if (psi != null) {
        ...
        if (parser instanceof PyParser) {
          final PyParser phizdetsParser = (PyParser)parser;
          phizdetsParser.setLanguageLevel(languageLevel);
          if (languageLevel == LanguageLevel.PYTHON26 && psi.getContainingFile().getName().equals("__builtin__.py")) {
            phizdetsParser.setFutureFlag(StatementParsing.FUTURE.PRINT_FUNCTION);
          }
        }
  ...
  @Nullable
  private ASTNode parseConsoleCode(@NotNull ASTNode node, PhizdetsConsoleData consoleData) {
  ...
  @Nullable
  private Lexer createConsoleLexer(ASTNode node, PhizdetsConsoleData consoleData) {
    if (consoleData.isIPhizdetsEnabled()) {
      return new PhizdetsConsoleLexer();
    }
    else {
