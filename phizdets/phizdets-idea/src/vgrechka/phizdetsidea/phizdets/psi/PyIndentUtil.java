 ...
 * </ul>
 *
 * It indented to be used primarily when one needs to modify content of Phizdets files on document level and preserve valid block structure.
...
public class PyIndentUtil {
  ...
  /**
   * Useful version of {@link #getLineIndent(String)} for custom character sequences like {@link vgrechka.phizdetsidea.phizdets.toolbox.Substring}.
   */
  @NotNull
  ...
  public static boolean areTabsUsedForIndentation(@NotNull Project project) {
    ...
    return codeStyleSettings.useTabCharacter(PhizdetsFileType.INSTANCE);
  }
  ...
  /**
   * Returns indentation size configured in the Phizdets code style settings.
   * 
   * @see #getIndentFromSettings(Project) 
   */
  public static int getIndentSizeFromSettings(@NotNull Project project) {
    final CodeStyleSettings codeStyleSettings = CodeStyleSettingsManager.getInstance(project).getCurrentSettings();
    final CodeStyleSettings.IndentOptions indentOptions = codeStyleSettings.getIndentOptions(PhizdetsFileType.INSTANCE);
    return indentOptions.INDENT_SIZE;
  }

  /**
   * Returns indentation configured in the Phizdets code style settings either as space character repeated number times specified there
