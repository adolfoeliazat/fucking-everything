...
import vgrechka.phizdetsidea.NotNullPredicate;
...
import vgrechka.phizdetsidea.phizdets.codeInsight.completion.OverwriteEqualsInsertHandler;
...
import vgrechka.phizdetsidea.phizdets.formatter.PyCodeStyleSettings;
import vgrechka.phizdetsidea.phizdets.magicLiteral.PyMagicLiteralTools;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyBuiltinCache;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyPsiUtils;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyStringLiteralExpressionImpl;
import vgrechka.phizdetsidea.phizdets.psi.impl.PhizdetsLanguageLevelPusher;
import vgrechka.phizdetsidea.phizdets.psi.resolve.PyResolveContext;
import vgrechka.phizdetsidea.phizdets.psi.resolve.QualifiedNameFinder;
import vgrechka.phizdetsidea.phizdets.psi.resolve.RatedResolveResult;
import vgrechka.phizdetsidea.phizdets.psi.stubs.PySetuptoolsNamespaceIndex;
import vgrechka.phizdetsidea.phizdets.psi.types.*;
import vgrechka.phizdetsidea.phizdets.refactoring.classes.PyDependenciesComparator;
import vgrechka.phizdetsidea.phizdets.refactoring.classes.extractSuperclass.PyExtractSuperclassHelper;
...
import static vgrechka.phizdetsidea.phizdets.psi.PyFunction.Modifier.CLASSMETHOD;
import static vgrechka.phizdetsidea.phizdets.psi.PyFunction.Modifier.STATICMETHOD;
...
public class PyUtil {
   ...
   * in {@link vgrechka.phizdetsidea.phizdets.psi.resolve.ImportedResolveResult}.
   */
  @NotNull
  ...
  /**
   * Returns Phizdets language level for a virtual file.
   *
  ...
  public static LanguageLevel getLanguageLevelForVirtualFile(@NotNull Project project,
    ...
    if (folder != null) {
      ...
      final LanguageLevel fileLevel = PhizdetsLanguageLevelPusher.getFileLanguageLevel(project, virtualFile);
      if (fileLevel != null) {
        return fileLevel;
      }
    }
  ...
  public static void invalidateLanguageLevelCache(@NotNull Project project) {
    project.putUserData(PhizdetsLanguageLevelPusher.PYTHON_LANGUAGE_LEVEL, null);
  }

  @NotNull
  public static LanguageLevel guessLanguageLevelWithCaching(@NotNull Project project) {
    LanguageLevel languageLevel = project.getUserData(PhizdetsLanguageLevelPusher.PYTHON_LANGUAGE_LEVEL);
    if (languageLevel == null) {
      languageLevel = guessLanguageLevel(project);
      project.putUserData(PhizdetsLanguageLevelPusher.PYTHON_LANGUAGE_LEVEL, languageLevel);
    }
  ...
  public static LanguageLevel guessLanguageLevel(@NotNull Project project) {
    ...
    if (moduleManager != null) {
      ...
      for (Module projectModule : moduleManager.getModules()) {
        final Sdk sdk = PhizdetsSdkType.findPhizdetsSdk(projectModule);
        if (sdk != null) {
          final LanguageLevel level = PhizdetsSdkType.getLanguageLevelForSdk(sdk);
  ...
  /**
   * Returns the line comment that immediately precedes statement list of the given compound statement. Phizdets parser ensures
  ...
  /**
   * If directory is a PsiDirectory, that is also a valid Phizdets package, return PsiFile that points to __init__.py,
   ...
   * @return PsiFile or PsiDirectory, if target is a Phizdets package and {@code null} null otherwise
   */
  @Nullable
  ...
  /**
   * If target is a Phizdets module named __init__.py file, return its directory. Otherwise return target unchanged.
  ...
  /**
   * Checks that given PsiDirectory can be treated as Phizdets package, i.e. it's either contains __init__.py or it's a namespace package
   * (effectively any directory in Phizdets 3.3 and above). Setuptools namespace packages can be checked as well, but it requires access to
   ...
   * @return whether given directory is Phizdets package
   *
  ...
  public static PyFile getOrCreateFile(String path, Project project) {
    ...
    if (vfile == null) {
      ...
      try {
        ...
        final FileTemplate template = fileTemplateManager.getInternalTemplate("Phizdets Script");
    ...
    if (!(psi instanceof PyFile)) {
      throw new IncorrectOperationException(PyBundle.message(
        "refactoring.move.module.members.error.cannot.place.elements.into.nonphizdets.file"));
    }
    return (PyFile)psi;
  }
  ...
  public static final class StringNodeInfo {
    ...
    public StringNodeInfo(@NotNull ASTNode node) {
      if (!PyTokenTypes.STRING_NODES.contains(node.getElementType())) {
        throw new IllegalArgumentException("Node must be valid Phizdets string literal token, but " + node.getElementType() + " was given");
      }
      myNode = node;
