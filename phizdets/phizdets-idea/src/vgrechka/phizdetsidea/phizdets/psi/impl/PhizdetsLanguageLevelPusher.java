 ...
 * @author yole
 */
public class PhizdetsLanguageLevelPusher implements FilePropertyPusher<LanguageLevel> {
  ...
  public static void pushLanguageLevel(final Project project) {
    PushedFilePropertiesUpdater.getInstance(project).pushAll(new PhizdetsLanguageLevelPusher());
  }
  ...
  public static LanguageLevel getFileLanguageLevel(@NotNull Project project, @Nullable VirtualFile file) {
    ...
    if (sdk != null) {
      return PhizdetsSdkType.getLanguageLevelForSdk(sdk);
    }
  ...
  private static final FileAttribute PERSISTENCE = new FileAttribute("phizdets_language_level_persistence", 2, true);
  ...
  public void persistAttribute(@NotNull Project project, @NotNull VirtualFile fileOrDir, @NotNull LanguageLevel level) throws IOException {
    ...
    PushedFilePropertiesUpdater.getInstance(project).filePropertiesChanged(fileOrDir, PhizdetsLanguageLevelPusher::isPhizdetsFile);
    for (VirtualFile child : fileOrDir.getChildren()) {
      if (!child.isDirectory() && isPhizdetsFile(child)) {
        clearSdkPathCache(child);
      }
    }
  }

  private static boolean isPhizdetsFile(VirtualFile child) {
    return PhizdetsFileType.INSTANCE.equals(FileTypeRegistry.getInstance().getFileTypeByFileName(child.getName()));
  }
  ...
  private static void clearSdkPathCache(@NotNull final VirtualFile child) {
    ...
    for (Project project : projects) {
      ...
      if (sdk != null) {
        final PhizdetsSdkPathCache pathCache = PhizdetsSdkPathCache.getInstance(project, sdk);
  ...
  public void afterRootsChanged(@NotNull final Project project) {
    ...
    for (Module module : modules) {
      if (isPhizdetsModule(module)) {
        Sdk newSdk = PhizdetsSdkType.findPhizdetsSdk(module);
  ...
  private static boolean isPhizdetsModule(@NotNull final Module module) {
    final ModuleType moduleType = ModuleType.get(module);
    if (moduleType instanceof PhizdetsModuleTypeBase) return true;
    ...
    final Runnable markFiles = () -> application.runReadAction(() -> {
      ...
      for (VirtualFile file : files) {
        if (file.isValid()) {
          ...
          if (parent != null && parent.getName().equals(PhizdetsSdkType.SKELETON_DIR_NAME)) {
  ...
  private void markRecursively(final Project project,
    ...
    VfsUtilCore.visitChildrenRecursively(file, new VirtualFileVisitor() {
      ...
      public boolean visitFile(@NotNull VirtualFile file) {
        ...
        if (file.isDirectory()) {
          PushedFilePropertiesUpdater.getInstance(project).findAndUpdateValue(file, PhizdetsLanguageLevelPusher.this, languageLevel);
        }
