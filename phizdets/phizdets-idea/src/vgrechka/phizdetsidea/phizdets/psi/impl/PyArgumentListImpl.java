...
import vgrechka.phizdetsidea.phizdets.FunctionParameter;
...
public class PyArgumentListImpl extends PyElementImpl implements PyArgumentList {
  ...
  public void addArgumentFirst(PyExpression arg) {
    ...
    if (pars.length == 0) {
      ...
      if (before != null && elementPrecedesElementsOfType(before, PhizdetsDialectsTokenSetProvider.INSTANCE.getExpressionTokens())) {
  ...
  public void addArgumentAfter(PyExpression argument, @Nullable PyExpression afterThis) {
    ...
    while (node != null) {
      ...
      else if (PhizdetsDialectsTokenSetProvider.INSTANCE.getExpressionTokens().contains(type)) {
