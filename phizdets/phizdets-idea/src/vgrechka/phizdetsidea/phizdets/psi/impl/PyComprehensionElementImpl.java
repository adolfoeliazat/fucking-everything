...
public abstract class PyComprehensionElementImpl extends PyElementImpl implements PyComprehensionElement {
  ...
  public PyExpression getResultExpression() {
    ASTNode[] exprs = getNode().getChildren(PhizdetsDialectsTokenSetProvider.INSTANCE.getExpressionTokens());
  ...
  private static ASTNode getNextExpression(ASTNode after) {
    ...
    while (node != null && !PhizdetsDialectsTokenSetProvider.INSTANCE.getExpressionTokens().contains(node.getElementType()));
    return node;
  }
