...
public class PyElementGeneratorImpl extends PyElementGenerator {
  ...
  public PsiFile createDummyFile(LanguageLevel langLevel, String contents, boolean physical) {
    ...
    final LightVirtualFile virtualFile = new LightVirtualFile(name, PhizdetsFileType.INSTANCE, contents);
    virtualFile.putUserData(LanguageLevel.KEY, langLevel);
    final PsiFile psiFile = ((PsiFileFactoryImpl)factory).trySetupPsiForFile(virtualFile, PhizdetsLanguage.getInstance(), physical, true);
    assert psiFile != null;
    return psiFile;
  }
  ...
  public static String getDummyFileName() {
    return "dummy." + PhizdetsFileType.INSTANCE.getDefaultExtension();
  }
