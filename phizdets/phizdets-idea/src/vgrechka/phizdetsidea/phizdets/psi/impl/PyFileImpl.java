...
public class PyFileImpl extends PsiFileBase implements PyFile, PyExpression {
  ...
  public PyFileImpl(FileViewProvider viewProvider) {
    this(viewProvider, PhizdetsLanguage.getInstance());
  }
  ...
  public FileType getFileType() {
    return PhizdetsFileType.INSTANCE;
  }
  ...
  public boolean isAcceptedFor(@NotNull Class visitorClass) {
    for (Language lang : getViewProvider().getLanguages()) {
      final List<PhizdetsVisitorFilter> filters = PhizdetsVisitorFilter.INSTANCE.allForLanguage(lang);
      for (PhizdetsVisitorFilter filter : filters) {
  ...
  public boolean processDeclarations(@NotNull final PsiScopeProcessor processor,
    ...
    if (remainingDunderAll != null) {
      for (String s : remainingDunderAll) {
        ...
        if (!processor.execute(new LightNamedElement(myManager, PhizdetsLanguage.getInstance(), s), resolveState)) return false;
      }
    }
    return true;
  }
