...
import vgrechka.phizdetsidea.phizdets.psi.resolve.PyResolveImportUtil;
import vgrechka.phizdetsidea.phizdets.psi.resolve.QualifiedNameFinder;
import vgrechka.phizdetsidea.phizdets.psi.stubs.PyClassStub;
...
import static vgrechka.phizdetsidea.phizdets.psi.impl.PyCallExpressionHelper.interpretAsModifierWrappingCall;
...
public class PyFunctionImpl extends PyBaseElementImpl<PyFunctionStub> implements PyFunction {
  ...
  public Icon getIcon(int flags) {
    ...
    if (property != null) {
      if (property.getGetter().valueOrNull() == this) {
        return PhizdetsIcons.Phizdets.PropertyGetter;
      }
      if (property.getSetter().valueOrNull() == this) {
        return PhizdetsIcons.Phizdets.PropertySetter;
      }
      if (property.getDeleter().valueOrNull() == this) {
        return PhizdetsIcons.Phizdets.PropertyDeleter;
      }
    ...
    return PhizdetsIcons.Phizdets.Function;
  }
  ...
  private boolean isGeneratedStub() {
    ...
    if (vFile != null) {
      ...
      if (vFile != null) {
        vFile = vFile.getParent();
        if (vFile != null && vFile.getName().equals(PhizdetsSdkType.SKELETON_DIR_NAME)) {
          return true;
        }
      }
    }
