...
import vgrechka.phizdetsidea.phizdets.codeInsight.regexp.PhizdetsVerboseRegexpLanguage;
...
public class PyStringLiteralExpressionImpl extends PyElementImpl implements PyStringLiteralExpression, RegExpLanguageHost {
  ...
  public PyType getType(@NotNull TypeEvalContext context, @NotNull TypeEvalContext.Key key) {
    ...
    if (nodes.size() > 0) {
      ...
      if (file != null) {
        IElementType type = PhizdetsHighlightingLexer.convertStringType(getStringNodes().get(0).getElementType(), text,
  ...
  private boolean isVerboseInjection() {
    ...
    if (files != null) {
      for (Pair<PsiElement, TextRange> file : files) {
        ...
        if (language == PhizdetsVerboseRegexpLanguage.INSTANCE) {
          return true;
        }
      }
    }
