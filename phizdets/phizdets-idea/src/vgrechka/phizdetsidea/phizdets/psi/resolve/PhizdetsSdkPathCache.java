 ...
 * @author yole
 */
public class PhizdetsSdkPathCache extends PhizdetsPathCache implements Disposable {
  private static final Key<Map<Project, PhizdetsSdkPathCache>> KEY = Key.create("PhizdetsPathCache");

  public static PhizdetsSdkPathCache getInstance(@NotNull Project project, @NotNull Sdk sdk) {
    synchronized (KEY) {
      Map<Project, PhizdetsSdkPathCache> cacheMap = sdk.getUserData(KEY);
      if (cacheMap == null) {
        cacheMap = new WeakHashMap<>();
        sdk.putUserData(KEY, cacheMap);
      }
      PhizdetsSdkPathCache cache = cacheMap.get(project);
      if (cache == null) {
        cache = new PhizdetsSdkPathCache(project, sdk);
  ...
  public PhizdetsSdkPathCache(@NotNull final Project project, @NotNull final Sdk sdk) {
    myProject = project;
    mySdk = sdk;
    ...
    connection.subscribe(ProjectJdkTable.JDK_TABLE_TOPIC, new ProjectJdkTable.Adapter() {
      ...
      public void jdkRemoved(Sdk jdk) {
        if (jdk == sdk) {
          Disposer.dispose(PhizdetsSdkPathCache.this);
        }
      }
    });
      ...
      public void rootSetChanged(RootProvider wrapper) {
        ...
        if (!project.isDisposed()) {
          ...
          for (Module module : modules) {
            PhizdetsModulePathCache.getInstance(module).clearCache();
          }
        }
        myBuiltins.set(null);
      }
  ...
  public void dispose() {
    if (mySdk != null) {
      synchronized (KEY) {
        final Map<Project, PhizdetsSdkPathCache> cacheMap = mySdk.getUserData(KEY);
        if (cacheMap != null) {
