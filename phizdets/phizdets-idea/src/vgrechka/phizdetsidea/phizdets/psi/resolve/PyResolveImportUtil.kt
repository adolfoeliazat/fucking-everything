...
import vgrechka.phizdetsidea.phizdets.psi.impl.PyImportResolver
import vgrechka.phizdetsidea.phizdets.pyi.PyiFile
import vgrechka.phizdetsidea.phizdets.sdk.PySdkUtil
import vgrechka.phizdetsidea.phizdets.sdk.PhizdetsSdkType

/**
 * Phizdets resolve utilities for qualified names.
 *
...
fun resolveQualifiedName(name: QualifiedName, context: PyQualifiedNameResolveContext): List<PsiElement> {
  ...
  val phizdetsResults = listOf(relativeResults,
                             resultsFromRoots(name, context),
                             relativeResultsFromSkeletons(name, context)).flatten()
  val allResults = foreignResults + phizdetsResults
  val results = if (name.componentCount > 0) foreignResults + findFirstResults(phizdetsResults) else allResults
...
fun fromFoothold(foothold: PsiElement): PyQualifiedNameResolveContext {
  ...
  return PyQualifiedNameResolveContextImpl(foothold.manager, module, foothold, PhizdetsSdkType.findPhizdetsSdk(module))
}
...
fun fromModule(module: Module): PyQualifiedNameResolveContext =
    PyQualifiedNameResolveContextImpl(PsiManager.getInstance(module.project), module, null, PhizdetsSdkType.findPhizdetsSdk(module))
  ...
  if (context.withoutRoots && footholdFile != null) {
    ...
    if (containingDirectory != null) {
      ...
      if (containingName != null && containingName.componentCount > 0) {
        ...
        val sdk = PhizdetsSdkType.findPhizdetsSdk(footholdFile) ?: return emptyList()
...
private fun isAcceptRootAsTopLevelPackage(context: PyQualifiedNameResolveContext): Boolean {
  context.module?.let {
    FacetManager.getInstance(it).allFacets.forEach {
      if (it is PhizdetsPathContributingFacet && it.acceptRootAsTopLevelPackage()) {
        return true
      }
    }
  }
...
private fun findCache(context: PyQualifiedNameResolveContext): PhizdetsPathCache? {
  return when {
    context.visitAllModules -> null
    context.module != null ->
      if (context.effectiveSdk != context.sdk) null else PhizdetsModulePathCache.getInstance(context.module)
    context.footholdFile != null -> {
      val sdk = PyBuiltinCache.findSdkForNonModuleFile(context.footholdFile)
      if (sdk != null) PhizdetsSdkPathCache.getInstance(context.project, sdk) else null
    }
    else -> null
  }
}
