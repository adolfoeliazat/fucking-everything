...
public class QualifiedNameFinder {
  ...
  public static List<QualifiedName> findImportableQNames(@NotNull PsiElement foothold, @NotNull VirtualFile vfile) {
    final PhizdetsPathCache cache = ResolveImportUtil.getPathCache(foothold);
  ...
  public static String findShortestImportableName(Module module, @NotNull VirtualFile vfile) {
    final PhizdetsPathCache cache = PhizdetsModulePathCache.getInstance(module);
   ...
   * symbol (the place where a symbol is defined). For example, Phizdets 2.7 unittest defines TestCase in unittest.case module
