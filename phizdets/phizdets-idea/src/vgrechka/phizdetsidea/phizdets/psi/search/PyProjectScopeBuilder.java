 ...
 */
package vgrechka.phizdetsidea.phizdets.psi.search;
...
public class PyProjectScopeBuilder extends ProjectScopeBuilderImpl {
   ...
   * In Phizdets, files in PYTHONPATH are library classes but not library sources, so the check in that method ensures that
   ...
   * includes them in project scope only if they are also marked as source roots. Phizdets code is often not marked as source
  ...
  /**
   * Calculates a search scope which excludes Phizdets standard library tests. Using such scope may be quite a bit slower than using
  ...
  private static GlobalSearchScope excludeSdkTestsScope(Project project, Sdk sdk) {
    if (sdk != null && sdk.getSdkType() instanceof PhizdetsSdkType) {
      ...
      if (libDir != null) {
        // superset of test dirs found in Phizdets 2.5 to 3.1
  ...
  public static VirtualFile findVirtualEnvLibDir(Sdk sdk) {
    ...
    if (homePath != null) {
      File root = PhizdetsSdkType.getVirtualEnvRoot(homePath);
      if (root != null) {
