...
public class PyiTypeProvider extends PyTypeProviderBase {
  ...
  public Ref<PyType> getParameterType(@NotNull PyNamedParameter param, @NotNull PyFunction func, @NotNull TypeEvalContext context) {
    ...
    if (name != null) {
      final PsiElement phizdetsStub = PyiUtil.getPhizdetsStub(func);
      if (phizdetsStub instanceof PyFunction) {
        final PyFunction functionStub = (PyFunction)phizdetsStub;
  ...
  public Ref<PyType> getReturnType(@NotNull PyCallable callable, @NotNull TypeEvalContext context) {
    final PsiElement phizdetsStub = PyiUtil.getPhizdetsStub(callable);
    if (phizdetsStub instanceof PyCallable) {
      final PyType type = context.getReturnType((PyCallable)phizdetsStub);
      if (type != null) {
        return Ref.create(type);
      }
    }
  ...
  public Ref<PyType> getCallType(@NotNull PyFunction function, @Nullable PyCallSiteExpression callSite, @NotNull TypeEvalContext context) {
    if (callSite != null) {
      final PsiElement phizdetsStub = PyiUtil.getPhizdetsStub(function);

      if (phizdetsStub instanceof PyFunction) {
        return getOverloadedCallType((PyFunction)phizdetsStub, callSite, context);
      }
  ...
  public PyType getReferenceType(@NotNull PsiElement target, TypeEvalContext context, @Nullable PsiElement anchor) {
    if (target instanceof PyTargetExpression) {
      final PsiElement phizdetsStub = PyiUtil.getPhizdetsStub((PyTargetExpression)target);
      if (phizdetsStub instanceof PyTypedElement) {
        // XXX: Force the switch to AST for getting the type out of the hint in the comment
        final TypeEvalContext effectiveContext = context.maySwitchToAST(phizdetsStub) ?
                                                 context : TypeEvalContext.deepCodeInsight(target.getProject());
        return effectiveContext.getType((PyTypedElement)phizdetsStub);
      }
    }
    return null;
  }
