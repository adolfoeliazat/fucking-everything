 ...
 */
package vgrechka.phizdetsidea.phizdets.refactoring.changeSignature;
...
public class PyChangeSignatureUsageProcessor implements ChangeSignatureUsageProcessor {
  ...
  public boolean processUsage(final ChangeInfo changeInfo,
    ...
    if (!isPhizdetsUsage(usageInfo)) return false;
  ...
  private static boolean isPhizdetsUsage(UsageInfo info) {
    final PsiElement element = info.getElement();
    if (element == null) return false;
    return element.getLanguage() == PhizdetsLanguage.getInstance();
  }


  @Override
  public boolean processPrimaryMethod(ChangeInfo changeInfo) {
    if (changeInfo instanceof PyChangeInfo && changeInfo.getLanguage().is(PhizdetsLanguage.getInstance())) {
