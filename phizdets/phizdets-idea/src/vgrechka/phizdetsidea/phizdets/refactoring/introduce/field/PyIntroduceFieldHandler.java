 ...
 */
package vgrechka.phizdetsidea.phizdets.refactoring.introduce.field;
...
import vgrechka.phizdetsidea.phizdets.inspections.quickfix.AddFieldQuickFix;
import vgrechka.phizdetsidea.phizdets.psi.*;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyFunctionBuilder;
...
public class PyIntroduceFieldHandler extends IntroduceHandler {
  ...
  private static boolean isTestClass(PsiFile file, Editor editor) {
    ...
    if (element1 != null) {
      ...
      if (clazz != null && PhizdetsUnitTestUtil.isTestCaseClass(clazz, null)) return true;
    }
    return false;
  }
  ...
  private static PsiElement addFieldToSetUp(PyClass clazz, final Function<String, PyStatement> callback) {
    final PyFunction init = clazz.findMethodByName(PhizdetsUnitTestUtil.TESTCASE_SETUP_NAME, false, null);
    if (init != null) {
      return AddFieldQuickFix.appendToMethod(init, callback);
    }
    final PyFunctionBuilder builder = new PyFunctionBuilder(PhizdetsUnitTestUtil.TESTCASE_SETUP_NAME, clazz);
  ...
  protected String getHelpId() {
    return "phizdets.reference.introduceField";
  }
  ...
  protected String getRefactoringId() {
    return "refactoring.phizdets.introduce.field";
  }
}
