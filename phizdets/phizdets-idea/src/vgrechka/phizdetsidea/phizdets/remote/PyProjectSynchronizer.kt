 ...
 * and phizdets (which may be remote).
 * This engine is sdk-specific and used by [vgrechka.phizdetsidea.phizdets.newProject.PhizdetsProjectGenerator] (and friends).
 *
 * When generator creates remote project, it may use phizdets helpers (with aid of tasks) and it may need some way
...
interface PyProjectSynchronizer {
   ...
   * @param syncDirection local-to-remote (aka java-to-phizdets) or opposite. See enum value doc.
   * @param callback code to be called after sync completion. Argument tells if sync was success or not.
   * @param fileNames files to be used as source (local files in case of java-to-phizdets, remote otherwise).
   ...
   * @param filePath local file name (in case of java-to-phizdets), remote otherwise
   */
