 ...
 */
public abstract class AbstractPhizdetsRunConfiguration<T extends AbstractPhizdetsRunConfiguration<T>> extends AbstractRunConfiguration
  implements AbstractPhizdetsRunConfigurationParams, CommandLinePatcher {
  ...
  public AbstractPhizdetsRunConfiguration(Project project, final ConfigurationFactory factory) {
  ...
  public PyCommonOptionsFormData getCommonOptionsFormData() {
    return new PyCommonOptionsFormData() {
      ...
      public Project getProject() {
        return AbstractPhizdetsRunConfiguration.this.getProject();
      }

      @Override
      public List<Module> getValidModules() {
        return AbstractPhizdetsRunConfiguration.this.getValidModules();
      }
  ...
  public final SettingsEditor<T> getConfigurationEditor() {
    final SettingsEditor<T> runConfigurationEditor = PhizdetsExtendedConfigurationEditor.create(createConfigurationEditor());
    ...
    PhizdetsRunConfigurationExtensionsManager.getInstance().appendEditors(this, (SettingsEditorGroup)group);
  ...
  private void checkExtensions() throws RuntimeConfigurationException {
    try {
      PhizdetsRunConfigurationExtensionsManager.getInstance().validateConfiguration(this, false);
    }
  ...
  private void checkSdk() throws RuntimeConfigurationError {
    if (PlatformUtils.isPyCharm()) {
      ...
      if (StringUtil.isEmptyOrSpaces(path)) {
        throw new RuntimeConfigurationError("Please select a valid Phizdets interpreter");
      }
    }
    else {
      if (!myUseModuleSdk) {
        if (StringUtil.isEmptyOrSpaces(getSdkHome())) {
          ...
          if (projectSdk == null || !(projectSdk.getSdkType() instanceof PhizdetsSdkType)) {
            throw new RuntimeConfigurationError(PyBundle.message("runcfg.unittest.no_sdk"));
          }
        }
        else if (!PhizdetsSdkType.getInstance().isValidSdkHome(getSdkHome())) {
          throw new RuntimeConfigurationError(PyBundle.message("runcfg.unittest.no_valid_sdk"));
        }
      }
      else {
        Sdk sdk = PhizdetsSdkType.findPhizdetsSdk(getModule());
        if (sdk == null) {
  ...
  public String getSdkHome() {
    ...
    if (StringUtil.isEmptyOrSpaces(mySdkHome)) {
      final Sdk projectJdk = PhizdetsSdkType.findPhizdetsSdk(getModule());
      if (projectJdk != null) {
  ...
  public Sdk getSdk() {
    if (myUseModuleSdk) {
      return PhizdetsSdkType.findPhizdetsSdk(getModule());
    }
    else {
      return PhizdetsSdkType.findSdkByPath(getSdkHome());
    }
  }
  ...
  public void readExternal(@NotNull Element element) throws InvalidDataException {
    ...
    // extension settings:
    PhizdetsRunConfigurationExtensionsManager.getInstance().readExternal(this, element);
  }
  ...
  public void writeExternal(@NotNull Element element) throws WriteExternalException {
    ...
    // extension settings:
    PhizdetsRunConfigurationExtensionsManager.getInstance().writeExternal(this, element);
  ...
  public static void copyParams(AbstractPhizdetsRunConfigurationParams source, AbstractPhizdetsRunConfigurationParams target) {
  ...
  protected void patchCommandLineForVirtualenv(GeneralCommandLine commandLine, String sdkHome) {
    PhizdetsSdkType.patchCommandLineForVirtualenv(commandLine, sdkHome, isPassParentEnvs());
  }
  ...
  public boolean excludeCompileBeforeLaunchOption() {
    ...
    return module != null ? ModuleType.get(module) instanceof PhizdetsModuleTypeBase : true;
  }
