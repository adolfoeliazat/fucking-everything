 ...
 * @author yole
 */
public class PhizdetsRunConfigurationProducer extends RunConfigurationProducer<PhizdetsRunConfiguration> {

  public PhizdetsRunConfigurationProducer() {
    super(PhizdetsConfigurationType.getInstance().getFactory());
  }

  @Override
  protected boolean setupConfigurationFromContext(PhizdetsRunConfiguration configuration,
  ...
  @Override
  public boolean isConfigurationFromContext(PhizdetsRunConfiguration configuration, ConfigurationContext context) {
    final Location location = context.getLocation();
    if (location == null) return false;
    final PsiFile script = location.getPsiElement().getContainingFile();
    if (!isAvailable(location, script)) return false;
  ...
  private static boolean isAvailable(@NotNull final Location location, @Nullable final PsiFile script) {
    if (script == null || script.getFileType() != PhizdetsFileType.INSTANCE) {
      return false;
    }
    final Module module = ModuleUtilCore.findModuleForPsiElement(script);
    if (module != null) {
