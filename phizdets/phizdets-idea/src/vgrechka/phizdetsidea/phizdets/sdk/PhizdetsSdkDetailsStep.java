...
import vgrechka.phizdetsidea.phizdets.packaging.PyCondaPackageService;
...
public class PhizdetsSdkDetailsStep extends BaseListPopupStep<String> {
  ...
  public static void show(final Project project,
    ...
    final PhizdetsSdkDetailsStep sdkHomesStep = new PhizdetsSdkDetailsStep(project, moreDialog, ownerComponent, existingSdks, sdkAddedCallback);
  ...
  public PhizdetsSdkDetailsStep(@Nullable final Project project,
  ...
  private void createLocalSdk() {
    SdkConfigurationUtil.createSdk(myProject, myExistingSdks, mySdkAddedCallback, false, false, PhizdetsSdkType.getInstance());
  }
  ...
  private void createVirtualEnvSdk() {
    ...
    Iterables.removeIf(allSdks, new Predicate<Sdk>() {
      ...
      public boolean apply(Sdk sdk) {
        return !(sdk.getSdkType() instanceof PhizdetsSdkType);
      }
    });
    final List<PhizdetsSdkFlavor> flavors = PhizdetsSdkFlavor.getApplicableFlavors(false);
    for (PhizdetsSdkFlavor flavor : flavors) {
      final Collection<String> strings = flavor.suggestHomePaths();
      for (String string : SdkConfigurationUtil.filterExistingPaths(PhizdetsSdkType.getInstance(), strings, myExistingSdks)) {
        ...
        public void virtualEnvCreated(Sdk sdk, boolean associateWithProject) {
          if (associateWithProject) {
            ...
            if (additionalData == null) {
              additionalData = new PhizdetsSdkAdditionalData(PhizdetsSdkFlavor.getFlavor(sdk.getHomePath()));
              ((ProjectJdkImpl)sdk).setSdkAdditionalData(additionalData);
            }
            if (myNewProject) {
              ((PhizdetsSdkAdditionalData)additionalData).associateWithNewProject();
            }
            else {
              ((PhizdetsSdkAdditionalData)additionalData).associateWithProject(myProject);
            }
          }
