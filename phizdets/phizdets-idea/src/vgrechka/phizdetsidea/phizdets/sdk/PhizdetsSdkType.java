...
import vgrechka.phizdetsidea.phizdets.packaging.PyCondaPackageManagerImpl;
import vgrechka.phizdetsidea.phizdets.psi.LanguageLevel;
import vgrechka.phizdetsidea.phizdets.psi.impl.PyBuiltinCache;
import vgrechka.phizdetsidea.phizdets.psi.search.PyProjectScopeBuilder;
 ...
 * @author yole
 */
public final class PhizdetsSdkType extends SdkType {
  public static final String REMOTE_SOURCES_DIR_NAME = "remote_sources";
  private static final Logger LOG = Logger.getInstance("#" + PhizdetsSdkType.class.getName());
  ...
  private static final String[] UNIX_BINARY_NAMES = new String[]{"jython", "pypy", "phizdets"};
  private static final String[] WIN_BINARY_NAMES = new String[]{"jython.bat", "ipy.exe", "pypy.exe", "python.exe"};

  private static final Key<WeakReference<Component>> SDK_CREATOR_COMPONENT_KEY = Key.create("#vgrechka.phizdetsidea.phizdets.sdk.creatorComponent");
  public static final Predicate<Sdk> REMOTE_SDK_PREDICATE = sdk -> isRemote(sdk);

  public static PhizdetsSdkType getInstance() {
    return SdkType.findInstance(PhizdetsSdkType.class);
  }

  private PhizdetsSdkType() {
    super("Phizdets SDK");
  }
  ...
  /**
   * @return name of builtins skeleton file; for Phizdets 2.x it is '{@code __builtins__.py}'.
   */
  @NotNull
  @NonNls
...
//    return null;
  ...
  @Nullable
  private static String findPhizdetsInPath() {
    final String defaultCommand = SystemInfo.isWindows ? "python.exe" : "phizdets";
  ...
  public boolean isValidSdkHome(@Nullable final String path) {
    return PhizdetsSdkFlavor.getFlavor(path) != null;
  }
  ...
  public Sdk getVirtualEnvBaseSdk(Sdk sdk) {
    if (isVirtualEnv(sdk)) {
      final PhizdetsSdkFlavor flavor = PhizdetsSdkFlavor.getFlavor(sdk);
      final String version = getVersionString(sdk);
      if (flavor != null && version != null) {
        for (Sdk baseSdk : getAllSdks()) {
          if (!isRemote(baseSdk)) {
            final PhizdetsSdkFlavor baseFlavor = PhizdetsSdkFlavor.getFlavor(baseSdk);
  ...
  /**
   * @param binaryPath must point to a Phizdets interpreter
  ...
  public static File getVirtualEnvRoot(@NotNull final String binaryPath) {
    ...
    if (bin != null) {
      ...
      if (rootPath != null) {
        ...
        // Phizdets 3.3 virtualenvs can be found as described in PEP 405
  ...
  public static void patchCommandLineForVirtualenv(GeneralCommandLine commandLine, String sdkHome, boolean passParentEnvironment) {
    ...
    if (virtualEnvRoot != null) {
      ...
      if (env.containsKey(PATH)) {
        pathValue = PhizdetsEnvUtil.appendToPathEnvVar(env.get(PATH), virtualenvBin);
      }
      else if (passParentEnvironment) {
        // append to PATH
        pathValue = PhizdetsEnvUtil.appendToPathEnvVar(System.getenv(PATH), virtualenvBin);
      }
      else {
  ...
  public void saveAdditionalData(@NotNull final SdkAdditionalData additionalData, @NotNull final Element additional) {
    if (additionalData instanceof PhizdetsSdkAdditionalData) {
      ((PhizdetsSdkAdditionalData)additionalData).save(additional);
    }
  }
  ...
  public SdkAdditionalData loadAdditionalData(@NotNull final Sdk currentSdk, final Element additional) {
    if (RemoteSdkCredentialsHolder.isRemoteSdk(currentSdk.getHomePath())) {
      PhizdetsRemoteInterpreterManager manager = PhizdetsRemoteInterpreterManager.getInstance();
      if (manager != null) {
        return manager.loadRemoteSdkData(currentSdk, additional);
      }
    }
    return PhizdetsSdkAdditionalData.load(currentSdk, additional);
  }
  ...
  public String sdkPath(@NotNull VirtualFile homePath) {
    ...
    PhizdetsSdkFlavor flavor = PhizdetsSdkFlavor.getFlavor(path);
    if (flavor != null) {
...
//    PhizdetsSdkUpdater.updateOrShowError(sdk, null, project, ownerComponent);
      ...
      notificationListener =
        new NotificationListener() {
          ...
          public void hyperlinkUpdate(@NotNull Notification notification, @NotNull HyperlinkEvent event) {
            final PhizdetsRemoteInterpreterManager manager = PhizdetsRemoteInterpreterManager.getInstance();
            if (manager != null) {
              try {
  ...
  public static List<String> getSysPathsFromScript(@NotNull String binaryPath) throws InvalidSdkException {
    ...
    GeneralCommandLine cmd = PhizdetsHelper.SYSPATH.newCommandLine(binaryPath, Lists.<String>newArrayList());
    ...
    if (!runResult.checkSuccess(LOG)) {
      throw new InvalidSdkException(String.format("Failed to determine Phizdets's sys.path value:\nSTDOUT: %s\nSTDERR: %s",
                                                  runResult.getStdout(),
                                                  runResult.getStderr()));
    }
...
//    return flavor != null ? flavor.getVersionString(sdkHome) : null;
  ...
  @Nullable
  public static Sdk findPhizdetsSdk(@Nullable Module module) {
    if (module == null) return null;
    final Sdk sdk = ModuleRootManager.getInstance(module).getSdk();
    if (sdk != null && sdk.getSdkType() instanceof PhizdetsSdkType) return sdk;
    final Facet[] facets = FacetManager.getInstance(module).getAllFacets();
    for (Facet facet : facets) {
      final FacetConfiguration configuration = facet.getConfiguration();
      if (configuration instanceof PhizdetsFacetSettings) {
        return ((PhizdetsFacetSettings)configuration).getSdk();
      }
    }
    return null;
  }
  ...
  @Nullable
  public static Sdk findPhizdetsSdk(@NotNull final PsiElement element) {
    return findPhizdetsSdk(ModuleUtilCore.findModuleForPsiElement(element));
  }
  ...
  public static boolean isStdLib(VirtualFile vFile, Sdk phizdetsSdk) {
    if (phizdetsSdk != null) {
      final VirtualFile libDir = PyProjectScopeBuilder.findLibDir(phizdetsSdk);
      if (libDir != null && VfsUtilCore.isAncestor(libDir, vFile, false)) {
        return isNotSitePackages(vFile, libDir);
      }
      final VirtualFile venvLibDir = PyProjectScopeBuilder.findVirtualEnvLibDir(phizdetsSdk);
      if (venvLibDir != null && VfsUtilCore.isAncestor(venvLibDir, vFile, false)) {
        return isNotSitePackages(vFile, venvLibDir);
      }
      final VirtualFile skeletonsDir = PySdkUtil.findSkeletonsDir(phizdetsSdk);
  ...
  @Nullable
  public static Sdk findPhizdets2Sdk(@Nullable Module module) {
    final Sdk moduleSDK = findPhizdetsSdk(module);
    if (moduleSDK != null && !getLanguageLevelForSdk(moduleSDK).isPy3K()) {
      return moduleSDK;
    }
    return findPhizdets2Sdk(getAllSdks());
  }

  @Nullable
  public static Sdk findPhizdets2Sdk(@NotNull List<Sdk> sdks) {
  ...
  @Nullable
  public static Sdk findLocalCPhizdets(@Nullable Module module) {
    final Sdk moduleSDK = findPhizdetsSdk(module);
    if (moduleSDK != null && !isRemote(moduleSDK) && PhizdetsSdkFlavor.getFlavor(moduleSDK) instanceof CPhizdetsSdkFlavor) {
      return moduleSDK;
    }
  ...
  public static List<Sdk> getAllLocalCPhizdetss() {
    return getAllSdks().stream().filter(REMOTE_SDK_PREDICATE.negate()).collect(Collectors.toList());
  }

  @Nullable
  public static String getPhizdetsExecutable(@NotNull String rootPath) {
  ...
  public static Sdk getSdk(@NotNull final PsiElement element) {
    return findPhizdetsSdk(element);
  }
