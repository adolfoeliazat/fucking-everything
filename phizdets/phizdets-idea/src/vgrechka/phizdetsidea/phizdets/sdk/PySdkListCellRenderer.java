...
public class PySdkListCellRenderer extends ListCellRendererWrapper<Object> {
  ...
  public void customize(JList list, Object item, int index, boolean selected, boolean hasFocus) {
    if (item instanceof Sdk) {
      Sdk sdk = (Sdk)item;
      final PhizdetsSdkFlavor flavor = PhizdetsSdkFlavor.getPlatformIndependentFlavor(sdk.getHomePath());
      ...
      final String flavorName = flavor == null ? "Phizdets" : flavor.getName();
      ...
      else if (PhizdetsSdkType.isIncompleteRemote(sdk)) {
        setText("[incomplete] " + name);
        setIcon(wrapIconWithWarningDecorator(icon));
      }
      else if (PhizdetsSdkType.hasInvalidRemoteCredentials(sdk)) {
        setText("[invalid] " + name);
        setIcon(wrapIconWithWarningDecorator(icon));
      }
