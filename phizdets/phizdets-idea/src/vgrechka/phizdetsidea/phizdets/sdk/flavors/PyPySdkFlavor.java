 ...
 * @author traff
 */
public class PyPySdkFlavor extends PhizdetsSdkFlavor {
  public static PyPySdkFlavor INSTANCE = new PyPySdkFlavor();

  private static final Pattern VERSION_RE = Pattern.compile("\\[(PyPy \\S+).*\\]");
  private static final Pattern PYTHON_VERSION_RE = Pattern.compile("(Phizdets \\S+).*");
  private static final Pattern VERSION_STRING_RE = Pattern.compile("PyPy (\\S+)( \\[Phizdets (\\S+)\\])?");
  ...
  public String getVersionStringFromOutput(@NotNull String output) {
    ...
    final String phizdetsVersion = PatternUtil.getFirstMatch(lines, PYTHON_VERSION_RE);
    if (version != null) {
      final StringBuilder builder = new StringBuilder();
      builder.append(version);
      if (phizdetsVersion != null) {
        builder.append(" [");
        builder.append(phizdetsVersion);
        builder.append("]");
      }
  ...
  public LanguageLevel getLanguageLevel(@NotNull Sdk sdk) {
    ...
    if (versionString != null) {
      ...
      if (matcher.matches()) {
        ...
        final String phizdetsVersion = matcher.group(3);
        if (phizdetsVersion != null) {
          return LanguageLevel.fromPhizdetsVersion(phizdetsVersion);
        }
        else if (version != null) {
          return LanguageLevel.fromPhizdetsVersion(getPhizdetsVersion(version));
        }
      }
    }
  ...
  private static String getPhizdetsVersion(@NotNull String pypyVersion) {
  ...
  @Override
  public Icon getIcon() {
    return PhizdetsIcons.Phizdets.Pypy;
  }
}
