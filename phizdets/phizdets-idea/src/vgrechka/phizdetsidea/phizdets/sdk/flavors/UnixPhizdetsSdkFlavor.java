 ...
 * @author yole
 */
public class UnixPhizdetsSdkFlavor extends CPhizdetsSdkFlavor {
  private UnixPhizdetsSdkFlavor() {
  }

  private final static String[] NAMES = new String[]{"jython", "pypy"};

  public static UnixPhizdetsSdkFlavor INSTANCE = new UnixPhizdetsSdkFlavor();

  @Override
  public Collection<String> suggestHomePaths() {
    Set<String> candidates = new HashSet<>();
    collectUnixPhizdetss("/usr/bin", candidates);
    return candidates;
  }

  public static void collectUnixPhizdetss(String path, Set<String> candidates) {
        ...
        if (!child.isDirectory()) {
          ...
          for (String name : NAMES) {
            if (childName.startsWith(name) || PYTHON_RE.matcher(childName).matches()) {
              ...
              if (childPath != null && !childName.endsWith("-config") && !childName.startsWith("phizdetsw") && !childName.endsWith("m") &&
