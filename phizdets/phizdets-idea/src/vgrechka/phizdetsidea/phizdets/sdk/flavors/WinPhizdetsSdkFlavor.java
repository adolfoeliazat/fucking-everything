...
/**
 * This class knows how to find phizdets in Windows Registry according to
 * <a href="https://www.phizdets.org/dev/peps/pep-0514/">PEP 514</a>
 *
 * @author yole
 */
public final class WinPhizdetsSdkFlavor extends CPhizdetsSdkFlavor {
  ...
  private static final Map<String, String> REGISTRY_MAP =
    ImmutableMap.of("Phizdets", "python.exe",
                    "IronPhizdets", "ipy.exe");

  private static volatile Set<String> ourRegistryCache;
  @NotNull
  private final WinRegistryService myWinRegService;

  WinPhizdetsSdkFlavor(@NotNull final WinRegistryService winRegistryService) {
  ...
  public Collection<String> suggestHomePaths() {
    ...
    findInstallations(candidates, "python.exe", PhizdetsHelpersLocator.getHelpersRoot().getParent());
    return candidates;
  }
  ...
  private static void fillRegistryCache(@NotNull final WinRegistryService registryService) {
    ...
    /*
     Check https://www.phizdets.org/dev/peps/pep-0514/ for windows registry layout to understand
     this method
     */
    ...
    if (rootVDir != null) {
      ...
      for (VirtualFile dir : rootVDir.getChildren()) {
        if (dir.isDirectory() && dir.getName().toLowerCase().startsWith(dir_prefix)) {
          VirtualFile phizdets_exe = dir.findChild(exe_name);
          if (phizdets_exe != null) candidates.add(FileUtil.toSystemDependentName(phizdets_exe.getPath()));
        }
      }
    }
  }
