...
public class PhizdetsNoseTestConfigurationProducer extends
                                                 PhizdetsTestLegacyConfigurationProducer {
  public PhizdetsNoseTestConfigurationProducer() {
    super(PhizdetsTestConfigurationType.getInstance().LEGACY_NOSETEST_FACTORY);
  }
  ...
  protected boolean isAvailable(@NotNull final Location location) {
    ...
    return (PhizdetsTestConfigurationsModel.PYTHONS_NOSETEST_NAME.equals(TestRunnerService.getInstance(module).getProjectConfiguration()) && sdk != null);
  }
  ...
  protected boolean isTestFunction(@NotNull final PyFunction pyFunction, @Nullable final AbstractPhizdetsLegacyTestRunConfiguration configuration) {
    return PhizdetsUnitTestUtil.isTestCaseFunction(pyFunction, true);
  }
}