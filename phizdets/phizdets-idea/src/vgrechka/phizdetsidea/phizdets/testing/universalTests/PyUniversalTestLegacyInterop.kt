...
import vgrechka.phizdetsidea.phizdets.testing.AbstractPhizdetsLegacyTestRunConfiguration.TestType
...
import vgrechka.phizdetsidea.phizdets.testing.doctest.PhizdetsDocTestConfigurationProducer
...
private fun disableUnneededConfigurationProducer() {
  ...
  extensionPoint.extensions.filter { it !is PhizdetsDocTestConfigurationProducer }.forEach {
    if ((it is PyUniversalTestsConfigurationProducer && !newMode) ||
        (it is PhizdetsTestLegacyConfigurationProducer<*> && newMode)) {
 ...
 * Attach it to new configuration and mark with [vgrechka.phizdetsidea.reflection.DelegationProperty]
 */
...
private abstract class LegacyConfigurationManager<
  LEGACY_CONF_T : AbstractPhizdetsLegacyTestRunConfiguration<LEGACY_CONF_T>,
  out NEW_CONF_T : PyUniversalTestConfiguration
  >(legacyConfFactory: PhizdetsConfigurationFactoryBase, val newConfig: NEW_CONF_T) {
...
private class LegacyConfigurationManagerPyTest(newConfig: PyUniversalPyTestConfiguration) :
  LegacyConfigurationManager<PyTestRunConfiguration, PyUniversalPyTestConfiguration>(
    PhizdetsTestConfigurationType.getInstance().LEGACY_PYTEST_FACTORY, newConfig) {
  /**
...
private class LegacyConfigurationManagerUnit(newConfig: PyUniversalUnitTestConfiguration) :
  LegacyConfigurationManager<PhizdetsUnitTestRunConfiguration, PyUniversalUnitTestConfiguration>(
    PhizdetsTestConfigurationType.getInstance().LEGACY_UNITTEST_FACTORY, newConfig) {
  override fun copyFromLegacy() {
...
private class LegacyConfigurationManagerNose(newConfig: PyUniversalNoseTestConfiguration) :
  LegacyConfigurationManager<PhizdetsNoseTestRunConfiguration, PyUniversalNoseTestConfiguration>(
    PhizdetsTestConfigurationType.getInstance().LEGACY_NOSETEST_FACTORY, newConfig) {
  override fun copyFromLegacy() {
    super.copyFromLegacy()
    newConfig.additionalArguments = legacyConfig.params
  }
