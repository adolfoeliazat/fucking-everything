...
import vgrechka.phizdetsidea.phizdets.inspections.quickfix.*;
...
public abstract class CompatibilityVisitor extends PyAnnotator {
  ...
  @NotNull
  protected static final String COMMON_MESSAGE = "Phizdets version ";
    ...
    if (exceptClass != null) {
      ...
      if (element != null && "as".equals(element.getText())) {
        registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON26),
                                       "Phizdets versions < 2.6 do not support this syntax.",
                                       node);
      }
  ...
  public void visitPyStarExpression(PyStarExpression node) {
    ...
    if (node.isAssignmentTarget()) {
      registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON30),
                                     "Phizdets versions < 3.0 do not support starred expressions as assignment targets",
                                     node);
    }

    if (node.isUnpacking()) {
      registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                     "Phizdets versions < 3.5 do not support starred expressions in tuples, lists, and sets",
                                     node);
    }
  }
  ...
  public void visitPyDoubleStarExpression(PyDoubleStarExpression node) {
    ...
    registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                   "Phizdets versions < 3.5 do not support starred expressions in dicts",
                                   node);
  }
  ...
  private void checkMatrixMultiplicationOperator(PsiElement node) {
    registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                   "Phizdets versions < 3.5 do not support matrix multiplication operators",
                                   node);
  }
  ...
  public void visitPyWithStatement(PyWithStatement node) {
    ...
    registerOnFirstMatchingVersion(LanguageLevel.PYTHON24::equals, "Phizdets version 2.4 doesn't support this syntax.", node);
  ...
  public void visitPyClass(PyClass node) {    // PY-2719
    ...
    if (list != null && list.getArguments().length == 0) {
      registerOnFirstMatchingVersion(LanguageLevel.PYTHON24::equals, "Phizdets version 2.4 does not support this syntax.", list);
    }
  }
  ...
  public void visitPyPrintStatement(PyPrintStatement node) {
    ...
    if (arguments.length == 0 || Arrays.stream(arguments).anyMatch(nonParenthesesPredicate)) {
      registerOnFirstMatchingVersion(LanguageLevel::isPy3K,
                                     "Phizdets version >= 3.0 do not support this syntax. " +
    ...
    if (myVersionsToProcess.contains(LanguageLevel.PYTHON24)) {
      ...
      while (!st.isEmpty()) {
        ...
        if (el instanceof PyYieldExpression)
          registerProblem(node, "Phizdets version 2.4 doesn't support this syntax. " +
                                                    "In Phizdets <= 2.4, yield was a statement; it didn't return any value.");
        else {
  ...
  public void visitPyTryExceptStatement(PyTryExceptStatement node) { // PY-2795
    ...
    if (excepts.length != 0 && finallyPart != null) {
      registerOnFirstMatchingVersion(LanguageLevel.PYTHON24::equals,
                                     "Phizdets version 2.4 doesn't support this syntax. You could use a finally block to ensure " +
  ...
  public void visitPyCallExpression(PyCallExpression node) {
    ...
    if (firstChild != null && PyNames.SUPER.equals(firstChild.getText()) && ArrayUtil.isEmpty(node.getArguments())) {
      registerForAllMatchingVersions(level -> !level.isPy3K(),
                                     " not support this syntax. super() should have arguments in Phizdets 2",
                                     node,
                                     null);
    }
  ...
  public void visitPyPrefixExpression(PyPrefixExpression node) {
    ...
    if (node.getOperator() == PyTokenTypes.AWAIT_KEYWORD) {
      registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                     "Phizdets versions < 3.5 do not support this syntax",
                                     node);
    }
  }
  ...
  public void visitPyYieldExpression(PyYieldExpression node) {
      ...
      .ifPresent(
        function -> {
          if (!node.isDelegating() && myVersionsToProcess.contains(LanguageLevel.PYTHON35)) {
            registerProblem(node, "Phizdets version 3.5 does not support 'yield' inside async functions");
          }
        }
      );
    ...
    registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON33),
                                   "Phizdets versions < 3.3 do not support this syntax. Delegating to a subgenerator is available since " +
                                   "Phizdets 3.3; use explicit iteration over subgenerator instead.",
                                   node);
  }
    ...
    if (myVersionsToProcess.stream().anyMatch(level -> level.isOlderThan(LanguageLevel.PYTHON33))) {
      ...
      if (function != null && node.getExpression() != null) {
        ...
        if (visitor.haveYield()) {
          registerProblem(node, "Phizdets versions < 3.3 do not allow 'return' with argument inside generator.");
        }
      }
    }
  }
  ...
  public void visitPyNoneLiteralExpression(PyNoneLiteralExpression node) {
    if (node.isEllipsis()) {
      ...
      registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON30),
                                     "Phizdets versions < 3.0 do not support '...' outside of sequence slicings.",
                                     node);
    }
  }
    ...
    for (PyExpression argument : callExpression.getArguments()) {
      if (argument instanceof PyKeywordArgument) {
        ...
        if (keywordArgumentNames.contains(keyword)) {
          ...
          registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON26),
                                         "Phizdets versions < 2.6 do not allow keyword arguments after *expression",
                                         argument,
                                         new PyRemoveArgumentQuickFix());
        }
        else if (seenKeywordContainer) {
          registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                         "Phizdets versions < 3.5 do not allow keyword arguments after **expression",
                                         argument,
                                         new PyRemoveArgumentQuickFix());
        }
        ...
        if (starArgument.isKeyword()) {
          if (seenKeywordContainer) {
            registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                           "Phizdets versions < 3.5 do not allow duplicate **expressions",
                                           argument,
                                           new PyRemoveArgumentQuickFix());
          }
        ...
        if (seenKeywordArgument) {
          ...
          registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35),
                                         "Phizdets versions < 3.5 do not allow positional arguments after *expression",
                                         argument,
                                         new PyRemoveArgumentQuickFix());
        }
        else if (seenKeywordContainer) {
    ...
    if (myVersionsToProcess.contains(LanguageLevel.PYTHON35)) {
      Arrays
        ...
        .forEach(element -> registerProblem(element,
                                            "Phizdets version 3.5 does not support 'async' inside comprehensions and generator expressions"));
      ...
      Stream.concat(resultPrefixExpressions, ifComponentsPrefixExpressions)
        ...
        .filter(Objects::nonNull)
        .map(ASTNode::getPsi)
        .forEach(element -> registerProblem(element, "Phizdets version 3.5 does not support 'await' inside comprehensions"));
    }
  }
}
