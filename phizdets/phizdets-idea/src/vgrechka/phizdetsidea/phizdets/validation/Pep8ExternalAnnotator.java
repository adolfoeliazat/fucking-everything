...
import vgrechka.phizdetsidea.phizdets.inspections.PyPep8Inspection;
import vgrechka.phizdetsidea.phizdets.inspections.quickfix.PyFillParagraphFix;
import vgrechka.phizdetsidea.phizdets.inspections.quickfix.ReformatFix;
import vgrechka.phizdetsidea.phizdets.inspections.quickfix.RemoveTrailingBlankLinesFix;
...
import vgrechka.phizdetsidea.phizdets.sdk.PreferredSdkComparator;
...
public class Pep8ExternalAnnotator extends ExternalAnnotator<Pep8ExternalAnnotator.State, Pep8ExternalAnnotator.Results> {
  ...
  public State collectInformation(@NotNull PsiFile file) {
    ...
    if (vFile == null || vFile.getFileType() != PhizdetsFileType.INSTANCE) {
      return null;
    }
    Sdk sdk = PhizdetsSdkType.findLocalCPhizdets(ModuleUtilCore.findModuleForPsiElement(file));
    if (sdk == null) {
  ...
  private static void reportMissingInterpreter() {
    ...
    List<Sdk> allSdks = PhizdetsSdkType.getAllSdks();
    Collections.sort(allSdks, PreferredSdkComparator.INSTANCE);
    for (Sdk sdk : allSdks) {
      LOG.info("  Path: " + sdk.getHomePath() + "; Flavor: " + PhizdetsSdkFlavor.getFlavor(sdk) + "; Remote: " + PhizdetsSdkType.isRemote(sdk));
    }
    LOG.info("]");
  }
  ...
  public Results doAnnotate(State collectedInfo) {
    ...
    GeneralCommandLine cmd = PhizdetsHelper.PYCODESTYLE.newCommandLine(collectedInfo.interpreterPath, options);
  ...
  public void apply(@NotNull PsiFile file, Results annotationResult, @NotNull AnnotationHolder holder) {
    ...
    for (Problem problem : annotationResult.problems) {
      ...
      if (problemElement != null) {
        // TODO Remove: a workaround until the bundled pycodestyle.py supports Phizdets 3.6 variable annotations by itself
  ...
  private static boolean ignoreDueToSettings(Project project, Problem problem, @Nullable PsiElement element) {
    ...
    final CommonCodeStyleSettings commonSettings = codeStyleSettings.getCommonSettings(PhizdetsLanguage.getInstance());
        ...
        if (matcher.matches()) {
          ...
          if (functionNearby || classNearby) {
            if (PyUtil.isTopLevel(element)) {
              ...
              // Blanks around classes have priority over blanks around functions as defined in Phizdets spacing builder
    ...
    if (problem.myCode.equals("W191") && codeStyleSettings.useTabCharacter(PhizdetsFileType.INSTANCE)) {
      return true;
    }
    return false;
  }
