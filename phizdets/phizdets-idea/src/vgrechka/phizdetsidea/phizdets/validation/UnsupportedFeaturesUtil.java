...
public class UnsupportedFeaturesUtil {
  ...
  private static void fillTestCaseMethods() throws IOException {
    ...
    final FileReader reader = new FileReader(PhizdetsHelpersLocator.getHelperPath("/tools/class_method_versions.xml"));
    try {
  ...
  private static void fillMaps() throws IOException {
    Logger log = Logger.getInstance(UnsupportedFeaturesUtil.class.getName());
    FileReader reader = new FileReader(PhizdetsHelpersLocator.getHelperPath("/tools/versions.xml"));
    try {
  ...
  private static class VersionsParser extends DefaultHandler {
    ...
    public void startElement(String namespaceURI,
      ...
      myContent.reset();
      if (localName.equals("phizdets")) {
        BUILTINS.put(LanguageLevel.fromPhizdetsVersion(attr.getValue("version")), new HashSet<>());
        MODULES.put(LanguageLevel.fromPhizdetsVersion(attr.getValue("version")), new HashSet<>());
        myCurrentLevel = LanguageLevel.fromPhizdetsVersion(attr.getValue("version"));
      }
     }
