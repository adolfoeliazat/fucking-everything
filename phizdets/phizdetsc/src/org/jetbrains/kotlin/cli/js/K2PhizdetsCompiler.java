...
import static org.jetbrains.kotlin.utils.PathUtil.KOTLIN_COMPILER_JAR;
import static org.jetbrains.kotlin.utils.PathUtil.getPathUtilJar;

public class K2PhizdetsCompiler extends CLICompiler<K2JSCompilerArguments> {
    ...
    public static void main(String... args) {
        doMain(new K2PhizdetsCompiler(), args);
    }
    ...
    protected void setupPlatformSpecificArgumentsAndServices(
        ...
        if (!arguments.noStdlib) {
            KotlinPaths kotlinPathsForCompiler = new Object() {
                final File NO_PATH = new File("<no_path>");
                final String KOTLIN_COMPILER_JAR = "kotlin-compiler-1.1.0.jar";

                @NotNull
                KotlinPaths getKotlinPathsForCompiler() {
                    return new KotlinPathsFromHomeDir(getCompilerPathForCompilerJar());
                }

                @NotNull
                File getCompilerPathForCompilerJar() {
...
//            KotlinPaths kotlinPathsForCompiler = PathUtil.getKotlinPathsForCompiler();
            libraries.add(0, kotlinPathsForCompiler.getJsStdLibJarPath().getAbsolutePath());
        }
