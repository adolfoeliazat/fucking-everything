...
public class TemporaryVariable {
    ...
    /*package*/ static TemporaryVariable create(@NotNull JsName temporaryName, @Nullable JsExpression initExpression) {
        ...
        if (initExpression != null) {
            JsNameRef left = temporaryName.makePHPVarRef();
//            JsNameRef left = temporaryName.makeRef();
            rhs = JsAstUtils.assignment(left, initExpression);
    ...
    public JsNameRef reference() {
        JsNameRef result = new JsNameRef(variableName).setKind(PHPNameRefKind.VAR);
