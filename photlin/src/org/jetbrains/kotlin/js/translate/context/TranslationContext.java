...
public class TranslationContext {
    ...
    public JsExpression getInnerReference(@NotNull DeclarationDescriptor descriptor) {
        ...
        if (descriptor instanceof VariableDescriptor) {
            ...
            if (containingDeclaration instanceof PackageFragmentDescriptor) {
                ...
                if (fqName.isRoot()) {
                    JsExpression globalRef = new PHPGlobalVarRef(getInnerNameForDescriptor(descriptor));
                    ShebangKt.setKotlinType(globalRef, variableDescriptor.getType());
                    return globalRef;
                }
            }
            JsExpression fuck = JsAstUtils.purePHPNameRefFqn(getInnerNameForDescriptor(descriptor), null);;
            return fuck;
        } else {
    ...
    public JsExpression getDispatchReceiver(@NotNull ReceiverParameterDescriptor descriptor) {
        ...
        if (descriptor.getValue() instanceof ExtensionReceiver) return phpThisRef();
    ...
    private JsExpression captureIfNeedAndGetCapturedName(DeclarationDescriptor descriptor) {
        if (usageTracker != null) {
            ...
            if (name != null) {
                ...
                // @fucking
                if (result instanceof JsNameRef) {
                    JsNameRef jsNameRef = (JsNameRef) result;
    ...
    public JsExpression getArgumentForClosureConstructor(@NotNull DeclarationDescriptor descriptor) {
        ...
        return getNameForDescriptor(descriptor).makePHPVarRef();
    }
