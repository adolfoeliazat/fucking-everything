...
class ClassTranslator private constructor(
    ...
    private fun translate() {
        ...
        val constructorFunction =
            descriptor.unsubstitutedPrimaryConstructor?.let {context.getFunctionObject(it) }
            ?: context.createRootScopedFunction(descriptor)
        constructorFunction.name = scope.declareName("__construct")
//        constructorFunction.name = context.getInnerNameForDescriptor(descriptor)

        val phpClass = PHPClass(context.getInnerNameForDescriptor(descriptor))
        descriptor.phpClass = phpClass
        phpClass.classDescriptor = descriptor
        phpClass.translationContext = context
        phpClass.constructorFunction = constructorFunction
        context.addDeclarationStatement(phpClass)
...
//        phpClass.statements += bodyVisitor.fieldStatements
...
//        context.addDeclarationStatement(constructorFunction.makeStmt())
    ...
    private fun createEnumInitFunction(): JsFunction {
        ...
        function.body.statements += PHPPlainCodeExpression{
            "static \$initialized = false; " +
            "if (\$initialized) return; " +
            "\$initialized = true;"}.makeStmt()
    ...
    private fun addMetadataObject() {
        context().addDeclarationStatement(JsSingleLineComment("TODO:vgrechka Implement class metadata    ac19c1fb-6ac8-4892-aff5-7cbda9f5ca31"))
//        context().addDeclarationStatement(JsAstUtils.assignment(createMetadataRef(), metadataLiteral).makeStmt())
    ...
    private fun generateSecondaryConstructor(classContext: TranslationContext, constructor: KtSecondaryConstructor) {
        ...
        val thisNameRef = thisName.makePHPVarRef()
        ...
        val createInstance = when (referenceToClass) {
            is JsNameRef -> JsNew(referenceToClass)
            else -> wtf("8b6b8413-529a-4749-858b-3570a692f29d")
        }
//        val createInstance = Namer.createObjectWithPrototypeFrom(referenceToClass)
        val instanceVar = JsAstUtils.assignment(thisNameRef, JsAstUtils.elvis(thisNameRef, createInstance)).makeStmt()
    ...
    private fun translatePropertiesAsConstructorParameters(classDeclarationContext: TranslationContext) {
        for (parameter in getPrimaryConstructorParameters(classDeclaration)) {
            ...
            if (descriptor != null) {
                val name = classDeclarationContext.getNameForDescriptor(descriptor)
                this.descriptor.phpClass.statements += JsVars(JsVars.JsVar(name, null)-{o->
                    o.visibility = "public"
                })
    ...
    private fun addObjectCache(statements: MutableList<JsStatement>) {
        ...
        statements += JsAstUtils.assignment(PHPGlobalVarRef(cachedInstanceName), phpThisRef()).makeStmt()
...
//        context().addDeclarationStatement(JsAstUtils.newVar(cachedInstanceName, JsLiteral.NULL))
        ...
        if (enumInitializerName == null) {
            val instanceCreatedCondition = JsAstUtils.not(JsAstUtils.isset(PHPGlobalVarRef(cachedInstanceName)))
        ...
        instanceFun.body.statements += JsReturn(PHPGlobalVarRef(cachedInstanceName))
    ...
    private fun generateBridgesToTraitImpl() {
        for ((key, value) in CodegenUtil.getNonPrivateTraitMethods(descriptor)) {
            if (!areNamesEqual(key, value)) {
                generateDelegateCall(descriptor, value, key, phpThisRef(), context())
            }
        }
    }
    ...
    private fun generateBridge(bridge: Bridge<FunctionDescriptor>) {
        ...
        generateDelegateCall(descriptor, fromDescriptor, toDescriptor, phpThisRef(), context())
    }
