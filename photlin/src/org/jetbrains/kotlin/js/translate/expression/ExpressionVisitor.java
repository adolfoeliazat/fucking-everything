...
import static photlinc.Php_astKt.newPHPVar;
...
public final class ExpressionVisitor extends TranslatorVisitor<JsNode> {
    ...
    public JsNode visitProperty(@NotNull KtProperty expression, @NotNull TranslationContext context) {
        ...
        JsVars res = newPHPVar(name, initializer);
        res.source(expression);
        KtExpression initExpr = expression.getInitializer();
        if (initExpr != null) {
            res.getVars().forEach((var) ->
                    ShebangKt.setKotlinType(var, context.bindingContext().getType(initExpr)));
        }
        return res;
    }
