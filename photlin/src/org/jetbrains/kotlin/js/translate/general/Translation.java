...
public final class Translation {
    ...
    private static JsNode doTranslateExpression(KtExpression expression, TranslationContext context) {
        try {
            JsNode res = expression.accept(new ExpressionVisitor(), context);
            ShebangKt.setKtExpression(res, expression);
            return res;
        }
    ...
    public static JsExpression translateAsExpression(
        ...
        if (jsNode instanceof  JsExpression) {
            // @fucking Get type KotlinType of KtExpression
...
//        statements.add(new JsReturn(rootPackageName.makePHPVarRef()));
    ...
    private static void defineModule(@NotNull TranslationContext context, @NotNull List<JsStatement> statements, @NotNull String moduleId) {
        ...
        if (rootPackageName != null) {
            JsNameRef jsNameRef = rootPackageName.makePHPVarRef();

//            statements.add(new PHPMethodCall(new PHPVarRef("Kotlin"), "defineModule", Arrays.asList(context.program().getStringLiteral(moduleId), jsNameRef)).makeStmt());
...
//                    jsNameRef).makeStmt());
    ...
    private static JsStatement generateCallToMain(
        ...
        JsArrayLiteral jsArrayLiteral = new JsArrayLiteral(toStringLiteralList(arguments, context.program()));
        JsExpression temp = context.defineTemporary(jsArrayLiteral);
        JsExpression argument = temp;
