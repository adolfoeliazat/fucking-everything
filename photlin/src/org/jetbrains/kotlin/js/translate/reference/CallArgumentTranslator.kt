...
class CallArgumentTranslator private constructor(
            ...
            if (actualArgument is VarargValueArgument) {
                ...
                if (hasSpreadOperator) {
                    if (isNativeFunctionCall) {
                        ...
                        concatArguments = prepareConcatArguments(arguments, translateResolvedArgument(actualArgument, argsToJsExpr, this, parameterDescriptor), context)
                    }
                    else {
                        result.addAll(translateVarargArgument(actualArgument,
                                                              argsToJsExpr,
                                                              actualArgument.arguments.size > 1,
                                                              isVarargTypePrimitive, context, this, parameterDescriptor))
                    }
                }
                else {
                    if (isNativeFunctionCall) {
                        result.addAll(translateResolvedArgument(actualArgument, argsToJsExpr, this, parameterDescriptor))
                    }
                    else {
                        result.addAll(translateVarargArgument(actualArgument, argsToJsExpr, true, isVarargTypePrimitive, context, this, parameterDescriptor))
                    }
                }
            }
            else {
    ...
    companion object {
        ...
        private fun translateResolvedArgument(
            ...
            val res = when (resolvedArgument) {
                is DefaultValueArgument -> listOf(Namer.getUndefinedExpression()-{o->
                    o.valueParameterDescriptor = parameterDescriptor
                })
                else -> resolvedArgument.arguments.map {translatedArgs[it]!!-{o->
                    o.valueParameterDescriptor = parameterDescriptor
                }}
            }
            return res
        ...
        private fun translateVarargArgument(
            ...
            val list = translateResolvedArgument(resolvedArgument, translatedArgs, callArgumentTranslator, parameterDescriptor)

            return if (shouldWrapVarargInArray) {
                val concatArguments = prepareConcatArguments(arguments, list, context)
        ...
        private fun prepareConcatArguments(arguments: List<ValueArgument>, list: List<JsExpression>, context: TranslationContext): MutableList<JsExpression> {
            ...
            if (lastArrayContent.size > 0) {
                val arr = JsArrayLiteral(lastArrayContent).apply {sideEffects = SideEffectKind.DEPENDS_ON_STATE}
                val temp = context.defineTemporary(arr)
                concatArguments.add(temp)
//                concatArguments.add(JsArrayLiteral(lastArrayContent).apply { sideEffects = SideEffectKind.DEPENDS_ON_STATE })
            }
