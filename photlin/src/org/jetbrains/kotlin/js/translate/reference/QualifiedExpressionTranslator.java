...
public final class QualifiedExpressionTranslator {
    ...
    public static JsNode translateQualifiedExpression(
        ...
        // @fucking KtReferenceExpression to type name
        if (selector instanceof KtReferenceExpression) {
            DeclarationDescriptor dd = context.bindingContext().get(BindingContext.REFERENCE_TARGET, ((KtReferenceExpression) expression.getSelectorExpression()));
            if (dd instanceof PropertyDescriptor) {
                if ("message".equals(dd.getName().getIdentifier())) {
                    PropertyDescriptor pd = (PropertyDescriptor) dd;
                    ReceiverParameterDescriptor drp = pd.getDispatchReceiverParameter();
                    if (drp != null) {
                        KotlinType type = drp.getType();
                        String shitFuck = ShebangKt.getTypeName(type);

                        if ("kotlin.Throwable".equals(shitFuck)) {
                            return new PHPMethodCall(receiver, "getMessage", Arrays.asList());
                        }
                    }
                }
            }
