...
public final class JsAstUtils {
    ...
    public static JsInvocation invokeKotlinFunction(@NotNull String name, @NotNull JsExpression... argument) {
        return new JsInvocation(new JsNameRef("Kotlin::" + name), argument);
//        return invokeMethod(Namer.kotlinObject(), name, argument);
    ...
    @NotNull
    public static JsInvocation invokeStaticMethod(@NotNull JsExpression thisObject, @NotNull String name, @NotNull JsExpression... arguments) {
        return new JsInvocation(pureFqn(name, thisObject).setKind(PHPNameRefKind.STATIC), arguments);
    }
    ...
    public static JsExpression charToString(@NotNull JsExpression expression) {
        return new JsInvocation(new JsNameRef("chr"), expression);
//        return new JsInvocation(new JsNameRef("fromCharCode", new JsNameRef("String")), expression);
    ...
    public static JsExpression longFromInt(@NotNull JsExpression expression) {
        ...
        return invokeStaticMethod(Namer.kotlinLong(), Namer.LONG_FROM_INT, expression);
    }
    ...
    public static JsBinaryOperation elvis(@NotNull JsExpression op1, @NotNull JsExpression op2) {
        JsBinaryOperation jsBinaryOperation = new JsBinaryOperation(JsBinaryOperator.OR, op1, op2);
        jsBinaryOperation.elvis = true;
    ...
    public static JsStatement assignmentToThisField(@NotNull String fieldName, @NotNull JsExpression right) {
        return assignment(new JsNameRef(fieldName, phpThisRef()), right).makeStmt();
    }
    ...
    @NotNull
    public static JsBinaryOperation stringConcat(@NotNull JsExpression left, @NotNull JsExpression right) {
        return new JsBinaryOperation(JsBinaryOperator.PHP_STRING_CONCAT, left, right);
    }
    ...
    public static JsBinaryOperation typeOfIs(@NotNull JsExpression expression, @NotNull JsStringLiteral string) {
        return equality(new JsInvocation(phpNameRef("gettype"), Arrays.asList(expression)), string); // new JsPrefixOperation(JsUnaryOperator.TYPEOF, expression), string);
//        return equality(new JsPrefixOperation(JsUnaryOperator.TYPEOF, expression), string);
    }

    @NotNull
    public static JsInvocation isset(@NotNull JsExpression expression) {
        return new JsInvocation(phpNameRef("isset"), Arrays.asList(expression));
    }
    ...
    public static JsStatement defineSimpleProperty(@NotNull String name, @NotNull JsExpression value) {
        return assignment(new JsNameRef(name, phpThisRef()), value).makeStmt();
    }
    ...
    public static JsNameRef pureFqn(@NotNull String identifier, @Nullable JsExpression qualifier) {
        ...
        if (qualifier instanceof JsNameRef) {
            ...
            if (jsNameRef.getIdent().equals("Kotlin")) {
                if (identifier.equals("PropertyMetadata")) {
                    result = new JsNameRef("kotlin__" + identifier);
                }
            }
        }

        if (result == null) {
            result = new JsNameRef(identifier, qualifier);
            if (qualifier instanceof JsNameRef) {
                JsNameRef jsNameRef = (JsNameRef) qualifier;
                if (jsNameRef.getIdent().equals("Kotlin")) {
                    result.setKind(PHPNameRefKind.STATIC);
                }
            }
        }
    ...
    public static JsNameRef pureStaticFqn(@NotNull String identifier, @Nullable JsExpression qualifier) {
...
//        if (identifier.toString().equals("isCharSequence")) {
...
//        }
        ...
        if (qualifier instanceof JsNameRef) {
            JsNameRef jsNameRef = (JsNameRef) qualifier;
            if (jsNameRef.getIdent().equals("Kotlin")) {
                if (identifier.getIdent().equals("isCharSequence")) {
                    result = new JsNameRef("Kotlin::isCharSequence");
                }
                else if (identifier.getIdent().equals("isChar")) {
                    result = new JsNameRef("Kotlin::isChar");
                }
            }
        }
        ...
        if (result == null) {
            result = new JsNameRef(identifier, qualifier);
            if (qualifier == null && identifier.getIdent().equals("_")) {
                result.setKind(PHPNameRefKind.VAR);
            }
        }
        ...
        if (result.getQualifier() == null) {
            if ("IllegalStateException".equals(result.getIdent())) {
                result.setName(StaticContext.current.rootScope.declareName("kotlin__IllegalStateException"));
            }
            else if ("AssertionError".equals(result.getIdent())) {
                result.setName(StaticContext.current.rootScope.declareName("kotlin__AssertionError"));
            }
        }
        return result;
    }
    ...
    public static JsExpression purePHPNameRefFqn(@NotNull JsName identifier, @Nullable JsExpression qualifier) {
        JsExpression result;
        if (qualifier == null) {
            result = new JsNameRef(identifier).setKind(PHPNameRefKind.VAR);
        } else {
            throw new UnsupportedOperationException("Implement me    707a2d89-75f5-4bf5-8795-62e0bba1e58d");
        }
        MetadataProperties.setSideEffects(result, SideEffectKind.PURE);
        return result;
    }
