...
//    val functionRef = JsNameRef(getNameForDescriptor(descriptor), prototypeRef)
    ...
    classDescriptor.phpClass.statements += function.makeStmt()
...
fun TranslationContext.addAccessorsToPrototype(
    ...
    containingClass.phpClass.constructorFunction.body.statements.add(
        0, JsInvocation(JsNameRef("__photlin_defineProperty", scope().declareName("this").makePHPVarRef()),
                        JsStringLiteral(propertyName.ident),
                        literal
    ).makeStmt())
//    containingClass.phpClass.constructorFunction.body.statements.add(0, defineProperty.makeStmt())
