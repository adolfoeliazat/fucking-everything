...
public class K2PHPCompiler extends CLICompiler<K2PHPCompilerArguments> {
    private static final Map<String, ModuleKind> moduleKindMap = new HashMap<String, ModuleKind>();

    static {
        moduleKindMap.put(K2PHPArgumentConstants.MODULE_PLAIN, ModuleKind.PLAIN);
        moduleKindMap.put(K2PHPArgumentConstants.MODULE_COMMONJS, ModuleKind.COMMON_JS);
        moduleKindMap.put(K2PHPArgumentConstants.MODULE_AMD, ModuleKind.AMD);
        moduleKindMap.put(K2PHPArgumentConstants.MODULE_UMD, ModuleKind.UMD);
    }

    public static void main(String... args) {
        doMain(new K2PHPCompiler(), args);
    }

    @NotNull
    @Override
    protected K2PHPCompilerArguments createArguments() {
        return new K2PHPCompilerArguments();
    }

    @NotNull
    @Override
    protected ExitCode doExecute(
            @NotNull K2PHPCompilerArguments arguments, @NotNull CompilerConfiguration configuration, @NotNull Disposable rootDisposable
    ) {
    ...
    protected void setupPlatformSpecificArgumentsAndServices(
            @NotNull CompilerConfiguration configuration, @NotNull K2PHPCompilerArguments arguments,
        ...
        if (!arguments.noStdlib) {
            // @hack
            libraries.add(0, "E:\\work\\k2php\\lib\\kotlin-stdlib-js.jar");
...
//            libraries.add(0, PathUtil.getKotlinPathsForCompiler().getJsStdLibJarPath().getAbsolutePath());
    ...
    private static MainCallParameters createMainCallParameters(String main) {
        if (K2PHPArgumentConstants.NO_CALL.equals(main)) {
